...
export interface SubscriptionLimitDefinition {
  /** Maximum number of active student accounts (-1 = unlimited). */
  maxStudents: number;
  /** Maximum number of teacher accounts (-1 = unlimited). */
  maxTeachers: number;
  /** Maximum number of courses (-1 = unlimited). */
  maxCourses: number;
  /** Object storage quota in bytes (-1 = unlimited). */
  storageBytes: number;
  /** Daily AI token allowance across all modules (-1 = unlimited). */
  aiTokensPerDay: number;
  /** Monthly AI credit units (-1 = unlimited). */
  aiCreditsPerMonth: number;
  /** Maximum number of concurrent live sessions. */
  maxConcurrentLiveSessions: number;
  /** Maximum number of sub-tenants / departments. */
  maxSubTenants: number;
  /** API requests per minute. */
  apiRateLimit: number;
  /** True if custom domain / white-label is available. */
  customDomain: boolean;
  /** True if advanced analytics dashboard is available. */
  analyticsEnabled: boolean;
  /** True if SSO / SAML integration is available. */
  ssoEnabled: boolean;
  /** True if dedicated account manager is assigned. */
  dedicatedSupport: boolean;
}

/** Per-plan resource and feature limits. */
export const SUBSCRIPTION_LIMITS: Record<SubscriptionPlan, SubscriptionLimitDefinition> = {
 ...
 */
export const RBAC_PERMISSIONS: Record<UserRole, Permission[]> = {
