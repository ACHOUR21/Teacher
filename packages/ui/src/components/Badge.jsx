 ...
 * ```
 */
const Badge = React.forwardRef(({ className, variant, size, withDot = false, children, ...props }, ref) => (<span ref={ref} className={cn(badgeVariants({ variant, size, className }))} {...props}>
      {withDot && (<span className={cn('mr-1.5 inline-block h-1.5 w-1.5 rounded-full', dotColour(variant))} aria-hidden="true"/>)}
      {children}
    </span>));
Badge.displayName = 'Badge';
// ─── Helpers ──────────────────────────────────────────────────────────────────
function dotColour(variant) {
    const map = {
