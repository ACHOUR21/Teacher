...
// ─── Size Map ─────────────────────────────────────────────────────────────────
const avatarSizeVariants = cva('relative inline-flex shrink-0 items-center justify-center overflow-hidden rounded-full select-none', {
    variants: {
        size: {
...
function Avatar({ src, name, size = 'md', online = false, className, alt }) {
    ...
    return (<span className={cn('relative inline-block shrink-0', className)}>
      <span className={cn(avatarSizeVariants({ size }), !showImage && colourClass)}>
        {showImage ? (<img src={src} alt={alt ?? name} onError={() => setImgError(true)} className="h-full w-full object-cover"/>) : (<span aria-label={alt ?? name} className="font-semibold text-white leading-none">
            {initials}
          </span>)}
      </span>

      {online && (<span aria-label="Online" className={cn('absolute bottom-0 right-0 rounded-full bg-green-500 ring-white', indicatorSizeMap[size ?? 'md'])}/>)}
    </span>);
}
