...
test.describe('Courses — new course form', () => {
    test('new course modal/page opens', async ({ authedPage: page }) => {
        await page.goto('/courses');
        const createBtn = page.getByRole('button', { name: /new course|create/i }).or(page.getByRole('link', { name: /new course/i }));
...
test.describe('My Learning', () => {
    test('my learning page loads', async ({ authedPage: page }) => {
        ...
        await page.waitForLoadState('networkidle');
        await expect(page.getByRole('heading', { name: /my learning|enrolled/i })).toBeVisible({ timeout: 10_000 });
    });
