...
export function VideoUpload({ value, onChange, folder = 'lessons', className }) {
    ...
    return (<div className={cn('space-y-1', className)}>
      ...
      {!value && state !== 'done' ? (<button type="button" disabled={state === 'requesting' || state === 'uploading'} onClick={() => inputRef.current?.click()} className={cn('w-full flex items-center justify-center gap-2 px-3 py-2.5 border-2 border-dashed rounded-lg text-xs transition-all', state === 'error'
          ...
          {state === 'requesting' ? (<><SpinIcon /> Preparing…</>) : state === 'uploading' ? (<><SpinIcon /> Uploading {progress}%</>) : state === 'error' ? (<><AlertCircle className="h-3.5 w-3.5"/> {error} — click to retry</>) : (<><Upload className="h-3.5 w-3.5"/> Upload video</>)}
        </button>) : null}
      ...
      {(value || state === 'done') && (<div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg">
...
function SpinIcon() {
    return (<svg className="h-3.5 w-3.5 animate-spin" viewBox="0 0 24 24" fill="none">
