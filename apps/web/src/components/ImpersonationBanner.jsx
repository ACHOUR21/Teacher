...
export function ImpersonationBanner() {
    ...
    return (<div className="w-full bg-orange-500 text-white px-4 py-2 flex items-center justify-between gap-3 z-50 sticky top-0" role="alert" aria-live="polite">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span className="text-sm font-medium truncate">
          ...
          {impersonationInfo.tenantName && (<> on behalf of <strong>{impersonationInfo.tenantName}</strong></>)}
        </span>
      </div>

      <button onClick={handleExitImpersonation} className="flex items-center gap-1.5 shrink-0 bg-white/20 hover:bg-white/30 transition-colors px-3 py-1 rounded-md text-sm font-medium whitespace-nowrap" aria-label="Exit impersonation session">
