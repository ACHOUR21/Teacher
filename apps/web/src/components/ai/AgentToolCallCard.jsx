...
export function AgentToolCallCard({ toolCall }) {
    const [expanded, setExpanded] = useState(false);
    const isDone = toolCall.output !== undefined;
    const label = TOOL_LABELS[toolCall.name] ?? toolCall.name.replace(/_/g, ' ');
    return (<div className={cn('rounded-xl border text-xs overflow-hidden transition-colors', isDone ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50')}>
      ...
      {expanded && (<div className="px-3 pb-3 space-y-2 border-t border-inherit pt-2">
          {Object.keys(toolCall.input).length > 0 && (<div>
