...
type Scene = 'dashboard' | 'courses' | 'login';

// ─── Device config ────────────────────────────────────────────────────────────

const DEVICES: { id: Device; label: string; icon: typeof Monitor; maxWidth: number }[] = [
...
const SCENES: { id: Scene; label: string; icon: typeof LayoutDashboard }[] = [
...
function DashboardScene({ v }: { v: PreviewValues }) {
  return (
    <div className="flex" style={{ minHeight: 240 }}>
      ...
      <div className="w-28 shrink-0 bg-gray-50 border-r border-gray-100 p-3 space-y-1.5">
        {['Dashboard', 'Courses', 'Students', 'Grades', 'Reports'].map((item, i) => (
          <div
            ...
            style={i === 0 ? { backgroundColor: v.secondaryColor, color: '#fff' } : { color: '#6b7280' }}
          >
            {item}
          </div>
        ))}
      ...
      <div className="flex-1 p-4 bg-white space-y-3 overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[['24', 'Courses'], ['142', 'Students'], ['98%', 'Pass Rate']].map(([val, lbl]) => (
            <div key={lbl} className="rounded-lg border border-gray-100 p-2 text-center">
...
function CoursesScene({ v }: { v: PreviewValues }) {
  const cards = [
  ...
  return (
    <div className="p-4 bg-white space-y-3" style={{ minHeight: 240 }}>
      ...
      <div className="space-y-2">
        {cards.map(c => (
          <div key={c.title} className="rounded-lg border border-gray-100 p-2.5 flex items-center gap-3">
...
function LoginScene({ v }: { v: PreviewValues }) {
  return (
    <div className="p-6 bg-gray-50 flex items-center justify-center" style={{ minHeight: 240 }}>
      <div className="w-full max-w-[200px] bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-3">
        ...
        <div
          className="w-full rounded-lg py-1.5 text-center text-[11px] font-medium text-white"
...
function ContrastBadge({ color, label }: { color: string; label: string }) {
...
function CssOutput({ v, onCopy }: { v: PreviewValues; onCopy: () => void }) {
  ...
  return (
    <div className="relative rounded-lg bg-gray-900 text-gray-100 text-[10px] font-mono p-3 overflow-auto max-h-32">
      <button
        onClick={copy}
...
export function WhiteLabelPreview({ values: v }: Props) {
  ...
  return (
    <div className="space-y-3">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  device === d.id ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600',
                )}
              >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  scene === s.id ? 'bg-white shadow-sm text-gray-900 font-medium' : 'text-gray-400 hover:text-gray-600',
                )}
              >
      ...
      {/* Preview frame */}
      <div className="overflow-hidden rounded-xl border border-gray-200 shadow-sm transition-all duration-300"
           style={{ maxWidth: selectedDevice.maxWidth === 9999 ? '100%' : selectedDevice.maxWidth }}>
        {/* Top nav */}
        <div
        ...
        {/* Footer */}
        <div
          className="px-4 py-1.5 text-[10px] text-white/80 text-center"
      ...
      <div className="space-y-1.5">
        <div className="flex gap-3">
          {[{ color: v.primaryColor, label: 'Primary' }, { color: v.secondaryColor, label: 'Secondary' }].map(({ color, label }) => (
