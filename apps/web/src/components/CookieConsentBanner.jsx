...
export function CookieConsentBanner() {
    ...
    async function persistConsents(prefs) {
    ...
    return (<div className="fixed bottom-0 left-0 right-0 z-50 p-4 sm:p-6">
      <div className="max-w-2xl mx-auto bg-popover border border-border rounded-2xl shadow-2xl">
        <div className="p-5">
          ...
          {/* Expanded controls */}
          {expanded && (<div className="mt-4 space-y-3 border-t border-border pt-4">
              {[
                {
                    key: 'necessary',
                    label: 'Necessary',
            ...
            ].map(item => (<div key={item.key} className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-medium text-foreground">{item.label}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{item.desc}</p>
                  </div>
                  <button disabled={item.disabled} onClick={() => !item.disabled && item.onChange?.(!item.value)} className={cn('relative flex-shrink-0 mt-0.5 w-9 h-5 rounded-full transition-colors focus:outline-none', item.value ? 'bg-primary' : 'bg-muted', item.disabled && 'opacity-60 cursor-not-allowed')}>
                    <span className={cn('absolute top-0.5 left-0.5 w-4 h-4 bg-background rounded-full shadow transition-transform', item.value ? 'translate-x-4' : 'translate-x-0')}/>
                  </button>
                </div>))}
            </div>)}

          <div className="flex flex-wrap items-center gap-2 mt-4">
            <button onClick={acceptAll} className="px-4 py-1.5 bg-primary text-primary-foreground text-xs font-medium rounded-lg hover:bg-primary/90 transition-colors">
              Accept all
            </button>
            {expanded ? (<button onClick={saveCustom} className="px-4 py-1.5 bg-muted text-foreground text-xs font-medium rounded-lg hover:bg-accent transition-colors">
                Save preferences
              </button>) : (<button onClick={acceptNecessary} className="px-4 py-1.5 bg-muted text-foreground text-xs font-medium rounded-lg hover:bg-accent transition-colors">
                Necessary only
              </button>)}
            <button onClick={() => setExpanded(v => !v)} className="ml-auto flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
