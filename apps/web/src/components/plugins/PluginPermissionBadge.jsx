...
export function PluginPermissionBadge({ permission, className }) {
    const meta = PERMISSION_META[permission];
    if (!meta) {
        return null;
    }
    const { icon: Icon, bg, text, border } = RISK_CONFIG[meta.risk];
    return (<span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium border', bg, text, border, className)} title={permission}>
      <Icon className="h-3 w-3 flex-shrink-0"/>
      {meta.label}
    </span>);
}
export function PluginPermissionList({ permissions, maxVisible = 4 }) {
    ...
    return (<div className="space-y-1.5">
      {hasHigh && (<div className="flex items-start gap-1.5 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-2 py-1.5">
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {overflow > 0 && (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 border border-gray-200">
