...
export default function StartPage() {
  ...
  // Step 1 form
  const schoolForm = useForm<SchoolData>({
  ...
  const onSchoolSubmit = (data: SchoolData) => {
    setSchoolData(data);
    setStep(1);
  };
        ...
        <div className="flex items-center gap-2 mb-8">
          {STEPS.map((label, i) => (
            <React.Fragment key={i}>
              ...
              {i < STEPS.length - 1 && (
                <div className={cn('flex-1 h-0.5 transition-all', i < step ? 'bg-green-400' : 'bg-gray-200')} />
              )}
            </React.Fragment>
          ))}
        </div>
          ...
          {step === 0 && (
            <form onSubmit={schoolForm.handleSubmit(onSchoolSubmit)} className="space-y-5">
                  ...
                  {TENANT_TYPES.map(({ value, label, icon: Icon, desc }) => {
                    ...
                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => schoolForm.setValue('tenantType', value as any)}
                        className={cn(
              ...
              <div>
                ...
                <input
                  {...schoolForm.register('tenantName')}
                  placeholder="e.g. Greenwood Academy"
                  className={cn(
                    'w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500',
                    schoolForm.formState.errors.tenantName ? 'border-red-400' : 'border-gray-200'
                  )}
                />
                {schoolForm.formState.errors.tenantName && (
                  <p className="mt-1 text-xs text-red-500">{schoolForm.formState.errors.tenantName.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center border rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 border-gray-200">
                  ...
                  <input
                    {...schoolForm.register('tenantSlug')}
                    placeholder="greenwood-academy"
                    className="flex-1 px-3 py-2 text-sm focus:outline-none"
                  />
                </div>
                {schoolForm.formState.errors.tenantSlug && (
                  <p className="mt-1 text-xs text-red-500">{schoolForm.formState.errors.tenantSlug.message}</p>
                )}
              </div>

              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 h-11 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors"
              >
            ...
            <form onSubmit={adminForm.handleSubmit(onAdminSubmit)} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={cn('w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500',
                      adminForm.formState.errors.firstName ? 'border-red-400' : 'border-gray-200')}
                  />
                  {adminForm.formState.errors.firstName && (
                    <p className="mt-1 text-xs text-red-500">{adminForm.formState.errors.firstName.message}</p>
                  )}
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="Smith"
                    className={cn('w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500',
                      adminForm.formState.errors.lastName ? 'border-red-400' : 'border-gray-200')}
                  />
                  {adminForm.formState.errors.lastName && (
                    <p className="mt-1 text-xs text-red-500">{adminForm.formState.errors.lastName.message}</p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={cn('w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500',
                    adminForm.formState.errors.email ? 'border-red-400' : 'border-gray-200')}
                />
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={cn('w-full px-3 py-2 pr-10 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500',
                      adminForm.formState.errors.password ? 'border-red-400' : 'border-gray-200')}
                  />
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={cn('w-full px-3 py-2 pr-10 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500',
                      adminForm.formState.errors.confirmPassword ? 'border-red-400' : 'border-gray-200')}
                  />
                ...
                {adminForm.formState.errors.confirmPassword && (
                  <p className="mt-1 text-xs text-red-500">{adminForm.formState.errors.confirmPassword.message}</p>
                )}
              </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 h-11 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700 disabled:opacity-60 transition-colors"
                >
                  {isSubmitting ? (
