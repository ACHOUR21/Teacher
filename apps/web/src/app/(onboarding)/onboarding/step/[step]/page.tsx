...
function Confetti() {
  const pieces = Array.from({ length: 40 }, (_, i) => ({
    ...
    color: ['#3B82F6', '#8B5CF6', '#10B981', '#F59E0B', '#EF4444', '#EC4899'][
      i % 6
    ],
...
function StepWelcome({ onNext }: { onNext: () => void }) {
  return (
    <div className="text-center space-y-8">
      ...
      <button
        onClick={onNext}
        className="inline-flex items-center gap-2 px-8 py-3.5 bg-blue-600 text-white rounded-xl text-base font-semibold hover:bg-blue-700 active:bg-blue-800 transition-colors shadow-md shadow-blue-200"
      >
...
const ORG_TYPES = ['School', 'University', 'Corporate', 'Other'] as const;
...
function StepOrgProfile({ onNext }: { onNext: () => void }) {
  const { orgName, orgType, country, timezone, updateOrg } =
    useOnboardingStore();
  ...
  const validate = () => {
    ...
    if (!timezone) e.timezone = 'Please select a timezone';
    setErrors(e);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (errors.orgName) setErrors((p) => ({ ...p, orgName: '' }));
            }}
        ...
        <div>
            ...
            {ORG_TYPES.map((type) => (
              <button
                ...
                onClick={() => {
                  ...
                  if (errors.orgType) setErrors((p) => ({ ...p, orgType: '' }));
                }}
                className={cn(
          ...
          {errors.orgType && (
            <p className="text-xs text-red-500 mt-1">{errors.orgType}</p>
          )}
        </div>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (errors.timezone) setErrors((p) => ({ ...p, timezone: '' }));
            }}
...
function StepBranding({ onNext }: { onNext: () => void }) {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-2">
              ...
              {logoUrl && (
                <button
                  type="button"
                  onClick={() => updateOrg({ logoUrl: null })}
        ...
        <div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {PRESET_COLORS.map((color) => (
              <button
                key={color}
                type="button"
                onClick={() => updateOrg({ primaryColor: color })}
                className={cn(
            ...
            <div className="relative">
              <input
                type="color"
                value={primaryColor}
                onChange={(e) => updateOrg({ primaryColor: e.target.value })}
                className="h-8 w-8 rounded-full cursor-pointer border border-gray-200 p-0.5 bg-transparent"
                title="Custom color"
              />
            </div>
          ...
          {/* Color preview */}
          <div
            className="mt-3 h-10 rounded-lg flex items-center justify-center text-white text-xs font-semibold"
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Empowering learners of tomorrow"
...
function StepInviteTeam({ onNext }: { onNext: () => void }) {
  ...
  const addEmail = () => {
    if (emails.length < 5) setEmails((p) => [...p, '']);
  };
  ...
  const handleNext = async () => {
    const newErrors = emails.map((e) =>
      e.trim() && !isValidEmail(e) ? 'Invalid email address' : ''
    );
    ...
    <div className="space-y-6">
          ...
          <div key={i} className="flex items-center gap-2">
            <div className="flex-1">
              <input
                ...
                placeholder={`admin${i + 1}@organization.com`}
                className={cn(
                  'w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors',
                  errors[i]
      ...
      <div className="flex items-center justify-between pt-2">
        <button
          onClick={onNext}
          disabled={loading}
          className="flex items-center gap-1.5 px-4 py-2.5 text-gray-500 text-sm hover:text-gray-700 transition-colors"
        >
...
function StepFirstCourse({ onNext }: { onNext: () => void }) {
  const { firstCourseTitle, firstCourseDescription, updateOrg } =
    useOnboardingStore();
  const [loading, setLoading] = useState(false);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            rows={3}
            value={firstCourseDescription}
            onChange={(e) =>
              updateOrg({ firstCourseDescription: e.target.value })
            }
...
function StepComplete() {
  ...
  return (
    <div className="relative text-center space-y-8 overflow-visible">
      {done && <Confetti />}
      ...
      <div className="space-y-4">
        <div className="relative">
          <div className="h-20 w-20 rounded-full bg-green-100 flex items-center justify-center mx-auto shadow-md">
            {loading ? (
              <SpinIcon className="h-10 w-10 text-green-600" />
            ) : (
      ...
      {!loading && summary.length > 0 && (
        <div className="bg-gray-50 rounded-xl border border-gray-200 divide-y divide-gray-100 text-left">
          {summary.map((item) => (
      ...
      {!loading && (
        <button
          ...
          className="w-full h-12 bg-blue-600 text-white rounded-xl font-semibold text-base hover:bg-blue-700 active:bg-blue-800 transition-colors shadow-md shadow-blue-200"
        >
          Go to Dashboard →
        </button>
      )}
