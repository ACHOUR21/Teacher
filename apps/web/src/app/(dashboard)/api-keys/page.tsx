...
const DEFAULT_KEY_FORM: CreateKeyForm = {
  name: '',
  scopes: ['read'],
  rateLimit: 1000,
  expiresAt: '',
...
const DEFAULT_WEBHOOK_FORM: CreateWebhookForm = {
  url: '',
  events: [],
};
...
export default function ApiKeysPage() {
  ...
  // Form state
  const [keyForm, setKeyForm] = useState<CreateKeyForm>(DEFAULT_KEY_FORM);
  const [webhookForm, setWebhookForm] = useState<CreateWebhookForm>(DEFAULT_WEBHOOK_FORM);
  ...
  const createKeyMutation = useMutation({
    mutationFn: (dto: CreateKeyForm) => api.post('/api-ecosystem/keys', dto).then(r => r.data),
    onSuccess: (res) => {
      queryClient.invalidateQueries({ queryKey: ['api-keys-list'] });
      queryClient.invalidateQueries({ queryKey: ['api-ecosystem-stats'] });
      const raw: string = res?.data?.rawKey ?? res?.rawKey ?? '';
  ...
  const revokeKeyMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/api-ecosystem/keys/${id}`).then(r => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['api-keys-list'] });
      queryClient.invalidateQueries({ queryKey: ['api-ecosystem-stats'] });
  ...
  const toggleKeyMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/api-ecosystem/keys/${id}/toggle`).then(r => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['api-keys-list'] });
      queryClient.invalidateQueries({ queryKey: ['api-ecosystem-stats'] });
    },
  ...
  const createWebhookMutation = useMutation({
    mutationFn: (dto: CreateWebhookForm) => api.post('/api-ecosystem/webhooks', dto).then(r => r.data),
    onSuccess: () => {
  ...
  const deleteWebhookMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/api-ecosystem/webhooks/${id}`).then(r => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['webhooks-list'] });
      queryClient.invalidateQueries({ queryKey: ['api-ecosystem-stats'] });
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() => { setKeyForm(DEFAULT_KEY_FORM); setCreateKeyOpen(true); }}
            >
              Create New Key
            </Button>
          </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              size="sm"
              leftIcon={<Plus className="h-4 w-4" />}
              onClick={() => { setWebhookForm(DEFAULT_WEBHOOK_FORM); setCreateWebhookOpen(true); }}
            >
              Add Webhook
            </Button>
          </div>
                  ...
                  webhooks.map(wh => (
                    <tr key={wh.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className={tdClass}>
                        <div className="flex flex-wrap gap-1">
                          {wh.events.map(ev => (
                            <span key={ev} className="text-xs bg-purple-50 text-purple-700 px-1.5 py-0.5 rounded font-medium">
                              {ev}
                            </span>
                          ))}
                        </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
                ...
                {ALL_SCOPES.map(scope => (
                  <label key={scope} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => setKeyForm(f => ({ ...f, scopes: toggleArrayItem(f.scopes, scope) }))}
            ...
            <div>
              ...
              <input
                ...
                min={1}
                onChange={e => setKeyForm(f => ({ ...f, rateLimit: Number(e.target.value) }))}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!keyForm.name.trim() || keyForm.scopes.length === 0}
      ...
      {/* ── Reveal Key Modal ─────────────────────────────────────────────────── */}
      <Dialog open={revealKeyOpen} onOpenChange={(open) => { if (!open) { setRevealKeyOpen(false); setRevealedKey(''); } }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  readOnly
                  value={revealedKey}
                  className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm font-mono bg-gray-50 focus:outline-none"
                />
                <Button
                  variant="outline"
                  size="sm"
          ...
          <DialogFooter>
            <Button
              size="sm"
              onClick={() => { setRevealKeyOpen(false); setRevealedKey(''); }}
            >
              Done
            </Button>
          </DialogFooter>
                ...
                {ALL_EVENTS.map(event => (
                  <label key={event} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => setWebhookForm(f => ({ ...f, events: toggleArrayItem(f.events, event) }))}
                      className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
