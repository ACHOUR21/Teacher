...
const DEFAULT_VALUES: WhiteLabelSettings = {
  brandName: '',
  customDomain: '',
  logoUrl: '',
  faviconUrl: '',
...
export default function WhiteLabelPage() {
  const {
    ...
    reset,
    formState: { isDirty },
  } = useForm<WhiteLabelSettings>({ defaultValues: DEFAULT_VALUES });

  const watchedValues = watch();

  // Load existing settings
  const { data: existing } = useQuery<WhiteLabelSettings>({
  ...
  const saveMutation = useMutation({
    mutationFn: (dto: WhiteLabelSettings) =>
      api.post('/white-label/settings', dto).then(r => r.data),
    onSuccess: () => {
  ...
  function onSubmit(values: WhiteLabelSettings) {
            ...
            <Card>
              ...
              <CardContent>
                <textarea
                  {...register('customCss')}
                  rows={6}
                  placeholder="/* Override styles here */"
