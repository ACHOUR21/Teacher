...
import { User, Palette, Bell, Shield, Key, Building, Eye, EyeOff, Check, BellRing, BellOff, Smartphone, Copy, ChevronRight, Users, CreditCard, } from 'lucide-react';
...
function SettingsNavCard({ card }) {
    const Icon = card.icon;
    return (<Link href={card.href} className="group block rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all duration-150">
...
function Input({ className, ...props }) {
    return (<input className={cn('w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white', className)} {...props}/>);
}
function PushNotificationToggle() {
    const { status, enable, disable } = usePushNotifications();
    if (status === 'unsupported') {
        return (<div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
    ...
    if (status === 'granted') {
        return (<div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
    ...
    return (<Button onClick={enable} variant="outline" size="sm" className="gap-2">
...
export default function SettingsPage() {
    ...
    const [newKeyForm, setNewKeyForm] = useState({ name: '', scopes: [] });
    ...
    const brandMutation = useMutation({
        mutationFn: (dto) => api.put('/white-label/branding', dto),
        onSuccess: () => toast.success('Branding saved'),
        onError: () => toast.error('Failed', 'Could not save branding.'),
    });
    const createKeyMutation = useMutation({
        mutationFn: (dto) => api.post('/api-ecosystem/keys', dto).then(r => r.data.data ?? r.data),
        onSuccess: (data) => {
            if (data?.key)
                {setShowKey(data.key);}
    ...
    const revokeKeyMutation = useMutation({
        mutationFn: (id) => api.delete(`/api-ecosystem/keys/${id}`),
    ...
    return (<div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {SETTINGS_CARDS.map((card) => (<SettingsNavCard key={card.href} card={card}/>))}
      </div>
      ...
      <div className="border-t border-gray-200 pt-6">
        ...
        <div className="flex gap-1 p-1 bg-gray-100 rounded-xl w-full overflow-x-auto">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-colors', activeTab === tab.id
        ...
        <div className="mt-4">
          ...
          {activeTab === 'general' && (<Card>
              ...
              <CardContent className="space-y-4">
                <Field label="Language">
                  <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" defaultValue="en" onChange={(e) => setLocale(e.target.value)}>
                ...
                <CardContent className="space-y-4">
                  <Field label="Current Password">
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setShowCurrentPw(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
                  ...
                  <Field label="New Password">
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setShowNewPw(p => !p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {(sessions ?? []).map((s) => (<div key={s.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                        <div>
                        ...
                        {!s.isCurrent && (<Button variant="ghost" size="sm" onClick={() => revokeSessionMutation.mutate(s.id)} className="text-red-600 hover:text-red-700 hover:bg-red-50">
                            Revoke
                          </Button>)}
                      </div>))}
          ...
          {activeTab === 'notifications' && (<Card>
            ...
            ].map(({ key, label, desc }) => (<label key={key} className="flex items-start justify-between gap-4 cursor-pointer py-2">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{label}</p>
                          <p className="text-xs text-gray-500">{desc}</p>
                        </div>
                        <div className="relative flex-shrink-0 mt-0.5">
                          <input type="checkbox" className="sr-only peer" checked={notifPrefs[key]} onChange={e => setNotifPrefs(p => ({ ...p, [key]: e.target.checked }))}/>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Primary Color">
                    <div className="flex gap-2">
                      <input type="color" value={brandForm.primaryColor} onChange={e => setBrandForm(f => ({ ...f, primaryColor: e.target.value }))} className="h-9 w-12 rounded border border-gray-200 cursor-pointer p-0.5"/>
                      <Input value={brandForm.primaryColor} onChange={e => setBrandForm(f => ({ ...f, primaryColor: e.target.value }))}/>
                    </div>
                  </Field>
                  <Field label="Secondary Color">
                    <div className="flex gap-2">
                      <input type="color" value={brandForm.secondaryColor} onChange={e => setBrandForm(f => ({ ...f, secondaryColor: e.target.value }))} className="h-9 w-12 rounded border border-gray-200 cursor-pointer p-0.5"/>
                ...
                <div className="rounded-xl border border-gray-200 p-4 bg-gray-50">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-lg flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: brandForm.primaryColor }}>
                      {brandForm.brandName?.[0]?.toUpperCase() ?? 'E'}
                    </div>
                    <span className="font-semibold text-gray-900">{brandForm.brandName || 'EduAI'}</span>
                    <button className="ml-auto px-4 py-1.5 rounded-lg text-white text-sm font-medium" style={{ backgroundColor: brandForm.primaryColor }}>
                      Sign Up
                    </button>
                    <button className="px-4 py-1.5 rounded-lg text-white text-sm font-medium" style={{ backgroundColor: brandForm.secondaryColor }}>
                      Learn More
                    </button>
                  </div>
                </div>
          ...
          {activeTab === 'api' && (<Card>
              ...
              <CardContent className="space-y-4">
                ...
                {showKey && (<div className="bg-green-50 border border-green-200 rounded-lg p-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => { void navigator.clipboard.writeText(showKey); toast.success('Copied'); }} className="text-green-600 hover:text-green-800 flex-shrink-0">
                        <Copy className="h-4 w-4"/>
                      </button>
                    </div>
                  </div>)}
                <div className="flex flex-col sm:flex-row gap-2">
                  <Input placeholder="Key name (e.g. Production Integration)" value={newKeyForm.name} onChange={e => setNewKeyForm(f => ({ ...f, name: e.target.value }))}/>
                ...
                <div className="mb-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SCOPE_OPTIONS.map(scope => (<label key={scope} className="flex items-center gap-1.5 text-sm cursor-pointer">
                        <input type="checkbox" checked={newKeyForm.scopes.includes(scope)} onChange={e => setNewKeyForm(f => ({
                    ...f,
                ...
                <div className="divide-y divide-gray-100">
                  {(apiKeys ?? []).map((key) => (<div key={key.id} className="py-3 flex items-center justify-between gap-4">
