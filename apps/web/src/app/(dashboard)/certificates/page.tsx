...
interface IssuedCertificate {
  ...
  verifyUrl?: string;
  credentialId?: string;
  template?: { name: string; course?: { title: string } | null };
...
const BORDER_STYLES = ['none', 'single', 'double', 'ornate'] as const;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit">
        {(['issued', 'templates'] as const).map(tab => (
          <button
            key={tab}
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <EmptyIssuedState searched={!!search} />
      ) : (
        <>
          {/* Card Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    {['Recipient', 'Course', 'Template', 'Issued Date', 'Verification Code', 'Actions'].map(h => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {certificates.map(cert => (
                    <IssuedCertificateRow key={cert.id} cert={cert} />
                  ))}
                </tbody>
              </table>
            </div>
...
function IssuedCertificateRow({ cert }: { cert: IssuedCertificate }) {
  const recipientName =
    cert.student?.user
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <EmptyTemplatesState onCreate={() => setShowEditor(true)} />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {templates.map(t => (
            <TemplateCard
      ...
      {showEditor && (
        <TemplateEditorModal onClose={() => setShowEditor(false)} />
      )}

      {issueTarget && (
        <IssueCertificateModal
          template={issueTarget}
...
function TemplateCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      {/* Preview Rectangle */}
      <div
        className="relative h-36 flex flex-col items-center justify-center gap-1 overflow-hidden"
      ...
      <div className="p-4">
        ...
        <button
          onClick={onIssue}
          className="w-full flex items-center justify-center gap-1.5 py-2 bg-blue-600 rounded-lg text-xs font-medium text-white hover:bg-blue-700 transition-colors"
        >
...
function TemplateEditorModal({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: (payload: typeof form) =>
      api.post('/certificates/templates', payload).then(r => r.data),
    onSuccess: () => {
...
function CertificatePreview({ form }: {
  ...
  return (
    <div
      className="rounded-xl overflow-hidden shadow-lg aspect-[1.41/1] relative flex flex-col items-center justify-center p-6"
      style={{ backgroundColor: form.backgroundColor }}
    >
      {/* Border overlays */}
      {form.borderStyle !== 'none' && (
        <div className="absolute inset-3 border-2 rounded pointer-events-none" style={{ borderColor: `${form.textColor}40` }} />
      )}
      {(form.borderStyle === 'double' || form.borderStyle === 'ornate') && (
        <div className="absolute inset-5 border rounded pointer-events-none" style={{ borderColor: `${form.textColor}25` }} />
      )}
      {form.borderStyle === 'ornate' && (
        <div className="absolute inset-7 border rounded pointer-events-none" style={{ borderColor: `${form.textColor}15` }} />
      )}
      ...
      <div className="relative text-center space-y-2 w-full">
        ...
        {form.showSignature && (
          <div className="pt-3 flex justify-center gap-8">
...
function IssueCertificateModal({
  ...
  const courses: CourseOption[] = (coursesData as CourseOption[]) ?? [];

  const mutation = useMutation({
    mutationFn: () =>
      api.post('/certificates/issue-by-template', {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
          ...
          <button
            ...
            disabled={!recipientId.trim() || mutation.isPending}
            className="px-5 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
  ...
  return (
    <div className={cn(
      ...
      <div className="relative h-32 bg-gradient-to-br from-blue-600 via-blue-700 to-purple-700 flex items-center justify-center overflow-hidden">
        ...
        {isExpired ? (
          <div className="absolute top-2 right-2 bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">Expired</div>
        ) : (
...
function EmptyIssuedState({ searched }: { searched: boolean }) {
  return (
