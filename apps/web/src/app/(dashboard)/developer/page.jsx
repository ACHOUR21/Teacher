...
function CopyButton({ text }) {
    ...
    return (<button onClick={handleCopy} title="Copy to clipboard" className="absolute top-3 right-3 p-1.5 rounded text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition-colors">
...
function CodeBlock({ code, lang }) {
    return (<div className="relative">
      <pre className={cn('rounded-lg bg-gray-900 text-gray-100 p-4 pr-12 text-sm font-mono overflow-x-auto', lang === 'bash' && 'text-green-300')}>
        <code>{code}</code>
      </pre>
    ...
    return (<div className="space-y-8 p-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {SNIPPETS.map((s, i) => (<button key={i} onClick={() => setActiveSnippet(i)} className={cn('px-3 py-1.5 rounded text-xs font-medium transition-colors', activeSnippet === i
          ...
          <CardContent className="p-0">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {SCOPES_DOC.map(row => (<tr key={row.scope} className="hover:bg-gray-50">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {RATE_LIMITS.map(row => (<tr key={row.plan} className="hover:bg-gray-50">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <a href="/settings/api-keys" className="flex items-center gap-1 text-sm text-blue-600 hover:underline">
              Manage keys
        ...
        <CardContent>
          {activeKeys.length === 0 ? (<div className="text-center py-8 text-sm text-gray-400">
              No active API keys.{' '}
              ...
              {activeKeys.map(key => (<div key={key.id} className="flex items-center justify-between rounded-lg border border-gray-100 px-4 py-3 hover:bg-gray-50">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {key.scopes.slice(0, 3).map(s => (<span key={s} className="text-xs bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded">
                        {s}
                      </span>))}
                    {key.scopes.length > 3 && (<span className="text-xs text-gray-400">+{key.scopes.length - 3} more</span>)}
                  </div>
                </div>))}
            </div>)}
        </CardContent>
      ...
      <Card>
        <CardContent className="flex items-center justify-between py-4">
          ...
          <a href="/api/docs" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-colors">
            Open Swagger Docs
