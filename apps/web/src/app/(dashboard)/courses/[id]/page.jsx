...
export default function CourseDetailPage() {
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="relative rounded-2xl overflow-hidden h-56">
        {course.thumbnailUrl ? (<img src={course.thumbnailUrl} alt={course.title} className="w-full h-full object-cover"/>) : (<div className="w-full h-full bg-gradient-to-br from-blue-600 to-purple-600"/>)}
        ...
        {(isAdmin || isTeacher) && (<div className="absolute top-4 right-4 flex gap-2">
            ...
            <button onClick={() => { if (confirm('Delete this course?')) {
            deleteMutation.mutate();
        } }} className="p-2 bg-red-500/80 backdrop-blur rounded-lg text-white hover:bg-red-600/80 transition-colors">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        ].map(stat => (<div key={stat.label} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Description */}
          {course.description && (<div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="font-semibold text-gray-900 mb-3">About this course</h2>
              <p className="text-sm text-gray-600 leading-relaxed">{course.description}</p>
            </div>)}

          {/* Sections */}
          {course.sections?.length > 0 && (<div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                {course.sections.map((section, si) => (<div key={section.id} className="border border-gray-100 rounded-lg overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-50">
                      {(section.lessons ?? []).map((lesson) => (<div key={lesson.id} onClick={() => isEnrolled && router.push(`/courses/${id}/learn?lesson=${lesson.id}`)} className={cn('px-4 py-2.5 flex items-center gap-3 transition-colors', isEnrolled ? 'cursor-pointer hover:bg-blue-50' : 'cursor-default hover:bg-gray-50')}>
                          {isEnrolled ? (lesson.completed ? (<CheckCircle className="h-4 w-4 text-green-500 flex-shrink-0"/>) : (<Play className="h-4 w-4 text-blue-500 flex-shrink-0"/>)) : (<Lock className="h-4 w-4 text-gray-300 flex-shrink-0"/>)}
                          <span className="text-sm text-gray-700">{lesson.title}</span>
                          {lesson.duration && (<span className="ml-auto text-xs text-gray-400">{lesson.duration}m</span>)}
                        </div>))}
                    </div>
                  </div>))}
              </div>
          ...
          {/* Tags */}
          {course.tags?.length > 0 && (<div className="flex flex-wrap gap-2">
              {course.tags.map((tag) => (<span key={tag} className="px-3 py-1 bg-blue-50 text-blue-700 text-xs rounded-full">{tag}</span>))}
            </div>)}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 sticky top-6">
            ...
            {isEnrolled ? (<>
              ...
              </>) : (<Button className="w-full mt-3" loading={enrollMutation.isPending} onClick={() => enrollMutation.mutate()}>
            ...
            <ul className="mt-4 space-y-2 text-sm text-gray-600">
              ...
              <li className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-gray-400"/>{course.sections?.reduce((a, s) => a + (s.lessons?.length ?? 0), 0) ?? 0} lessons</li>
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-gray-400"/>Certificate of completion</li>
            </ul>
          </div>

          {/* Instructor */}
          {course.teacher && (<div className="bg-white rounded-xl border border-gray-200 p-5">
