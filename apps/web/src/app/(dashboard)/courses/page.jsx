...
function StarRating({ rating }) {
    return (<span className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={cn('h-3 w-3', i < Math.round(rating) ? 'fill-yellow-400 text-yellow-400' : 'fill-gray-200 text-gray-200')}/>))}
...
function CourseCard({ enrollment }) {
    ...
    return (<Link href={`/courses/${course.id}/learn`} className="group block bg-card rounded-xl border border-border overflow-hidden hover:shadow-md hover:border-primary/30 transition-all duration-200">
      {/* Thumbnail */}
      ...
      <div className="p-4">
        {course.category && (<p className="text-xs text-blue-600 font-medium mb-1">{course.category}</p>)}
        ...
        {course.teacher && (<p className="text-xs text-muted-foreground mb-2">
        ...
        <div className="mb-2">
          <div className="flex justify-between text-xs text-muted-foreground mb-1">
            ...
            {course.totalLessons && (<span className="flex items-center gap-1">
                <Clock className="h-3 w-3"/>
                {course.totalLessons} lessons
              </span>)}
          </div>
          <div className="h-1.5 bg-muted rounded-full overflow-hidden">
            <div className={cn('h-full rounded-full transition-all', isComplete ? 'bg-green-500' : 'bg-primary')} style={{ width: `${pct}%` }}/>
          </div>
        </div>

        <div className="flex items-center justify-between mt-2 text-xs text-muted-foreground">
          <StarRating rating={course.rating ?? 0}/>
          {(durationH > 0 || durationM > 0) && (<span className="flex items-center gap-1">
              <Clock className="h-3 w-3"/>
              {durationH > 0 ? `${durationH}h` : ''}{durationM > 0 ? ` ${durationM}m` : ''}
            </span>)}
        </div>

        {lastAccessed && (<p className="text-xs text-muted-foreground/70 mt-1">Last accessed {lastAccessed}</p>)}
      </div>
    </Link>);
}
function SkeletonCard() {
    return (<div className="bg-card rounded-xl border border-border overflow-hidden animate-pulse">
...
export default function CoursesPage() {
    ...
    const filtered = enrollments.filter((e) => {
        if (activeTab === 'in-progress') {
            return !e.completedAt && (e.progressPct ?? 0) > 0;
        }
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-border">
        {TABS.map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors', activeTab === tab.id
                ? 'border-primary text-primary'
                : 'border-transparent text-muted-foreground hover:text-foreground hover:border-border')}>
            {tab.label}
            {!isLoading && (<span className={cn('ml-1.5 text-xs px-1.5 py-0.5 rounded-full', activeTab === tab.id ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground')}>
                {tab.id === 'all'
                    ? enrollments.length
                    : tab.id === 'in-progress'
      ...
      {isLoading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        </div>) : filtered.length === 0 ? (<div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-gray-200 rounded-2xl text-center">
          ...
          {filtered.map(enrollment => (<CourseCard key={enrollment.id} enrollment={enrollment}/>))}
        </div>)}
    </div>);
}
