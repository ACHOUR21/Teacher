...
export default function AnalyticsPage() {
    ...
    const totalAiRequests = (aiUsage ?? []).reduce((s, u) => s + (u._count?.id ?? 0), 0);
    const totalAiCost = (aiUsage ?? []).reduce((s, u) => s + Number(u._sum?.cost ?? 0), 0);
    const heatmapMax = Math.max(1, ...(engagement ?? []).map((e) => e.count));
    const heatmapByKey = {};
    (engagement ?? []).forEach((e) => { heatmapByKey[`${e.day}-${e.hour}`] = e.count; });
    return (<div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
          {RANGES.map(r => (<button key={r.days} onClick={() => setRange(r.days)} className={cn('px-3 py-1.5 rounded-md text-sm font-medium transition-colors', range === r.days ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700')}>
              {r.label}
            </button>))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <SparklineCard title="Total Users" value={overview?.totalUsers ?? 0} change="+12% vs last period" trend="up" sparkData={(userGrowth ?? []).map((d) => ({ value: d.count ?? 0 }))} sparkColor="#2563EB" icon={<Users className="h-5 w-5 text-blue-600"/>} iconBg="bg-blue-100" loading={overviewLoading}/>
        <SparklineCard title="Published Courses" value={overview?.totalCourses ?? 0} change="+4% vs last period" trend="up" sparkColor="#7C3AED" icon={<BookOpen className="h-5 w-5 text-purple-600"/>} iconBg="bg-purple-100" loading={overviewLoading}/>
        <SparklineCard title="Total Revenue" value={`$${Number(overview?.totalRevenue ?? 0).toFixed(0)}`} change="+8% vs last period" trend="up" sparkData={(revenue ?? []).map((d) => ({ value: d.revenue ?? 0 }))} sparkColor="#059669" icon={<DollarSign className="h-5 w-5 text-green-600"/>} iconBg="bg-green-100" loading={overviewLoading || revenueLoading}/>
        <SparklineCard title="AI Requests" value={totalAiRequests} change="+23% vs last period" trend="up" sparkColor="#D97706" icon={<Zap className="h-5 w-5 text-orange-600"/>} iconBg="bg-orange-100" loading={aiLoading}/>
      </div>

      {/* Tabs */}
      <div className="border-b border-gray-200">
        <nav className="flex gap-1 overflow-x-auto">
          {TABS.map(({ id, label, icon: Icon }) => (<button key={id} onClick={() => setTab(id)} className={cn('flex items-center gap-2 px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors', tab === id ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700')}>
      ...
      {tab === 'overview' && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <ExportButton data={userGrowth ?? []} filename="user-growth" label="Export"/>
              </div>
            </CardHeader>
            <CardContent>
              {growthLoading ? <ChartSkeleton height={220}/> : (<ResponsiveContainer width="100%" height={220}>
          ...
          <Card>
            ...
            ].map(({ label, value }) => (<div key={label} className="text-center p-3 bg-gray-50 rounded-lg">
          ...
          <Card>
            ...
            <CardContent>
              {revenueLoading ? <ChartSkeleton height={220}/> : (<ResponsiveContainer width="100%" height={220}>
          ...
          <Card>
            ...
            <CardContent>
              {aiLoading ? <ChartSkeleton height={220}/> : (<ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={aiUsage ?? []} cx="50%" cy="50%" outerRadius={75} innerRadius={40} dataKey="_count.id" nameKey="module" label={({ module }) => module?.replace(/_/g, ' ')?.slice(0, 8)}>
      ...
      {/* Courses Tab */}
      {tab === 'courses' && (<div className="space-y-5">
          {/* ComposedChart: enrollments (bar) + completion rate (line) */}
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>Enrollment vs Completion Rate</CardTitle>
                <ExportButton data={(topCourses ?? []).map((c) => ({
                title: c.title,
                enrollments: c.enrollCount,
                completionRate: c.completionRate,
            }))} filename="enrollment-vs-completion" label="Export"/>
              </div>
            </CardHeader>
            <CardContent>
              {coursesLoading ? <ChartSkeleton height={260}/> : (<ResponsiveContainer width="100%" height={260}>
                  <ComposedChart data={(topCourses ?? []).slice(0, 8)}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                    <XAxis dataKey="title" tick={{ fontSize: 9 }} tickFormatter={(t) => t.length > 12 ? t.slice(0, 12) + '…' : t}/>
                    <YAxis yAxisId="left" tick={{ fontSize: 10 }} orientation="left"/>
                    <YAxis yAxisId="right" tick={{ fontSize: 10 }} orientation="right" tickFormatter={v => `${v}%`} domain={[0, 100]}/>
                    <Tooltip content={<ChartTooltip valueFormatter={(v, name) => name === 'Completion %' ? `${v}%` : String(v)}/>}/>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(topCourses ?? []).map((c, i) => (<tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    {!topCourses?.length && (<tr><td colSpan={6} className="py-8 text-center text-gray-400">No course data yet</td></tr>)}
                  </tbody>
                </table>
              </div>
            </CardContent>
              ...
              <CardContent>
                {coursesLoading ? <ChartSkeleton height={260}/> : (<ResponsiveContainer width="100%" height={260}>
                    <BarChart data={(topCourses ?? []).slice(0, 8)} layout="vertical">
                      ...
                      <YAxis dataKey="title" type="category" tick={{ fontSize: 9 }} width={130} tickFormatter={(t) => t.length > 18 ? t.slice(0, 18) + '…' : t}/>
      ...
      {tab === 'students' && (<div className="space-y-5">
          <Card>
            ...
            <CardContent>
              ...
              <div className="overflow-x-auto">
                <div className="min-w-max">
                  <div className="flex gap-1 mb-1 pl-10">
                    {Array.from({ length: 24 }, (_, h) => (<div key={h} className="w-6 text-center text-xs text-gray-400">
                        {h % 6 === 0 ? `${h}h` : ''}
                      </div>))}
                  </div>
                  {DAYS.map((day, d) => (<div key={day} className="flex items-center gap-1 mb-1">
                      <div className="w-8 text-right text-xs text-gray-400 mr-1">{day}</div>
                      {Array.from({ length: 24 }, (_, h) => {
                    const count = heatmapByKey[`${d}-${h}`] ?? 0;
                    const intensity = count / heatmapMax;
                    return (<div key={h} title={`${day} ${h}:00 — ${count} activities`} className="w-6 h-6 rounded-sm transition-colors" style={{
              ...
              <div className="flex items-center gap-2 mt-4 text-xs text-gray-500">
                <span>Less</span>
                {[0.1, 0.3, 0.5, 0.7, 1].map(i => (<div key={i} className="w-4 h-4 rounded-sm" style={{ backgroundColor: `rgba(37,99,235,${i})` }}/>))}
                <span>More</span>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            ].map(({ label, value, icon: Icon, color }) => (<div key={label} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                    <div className={cn('h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0', color === 'blue' ? 'bg-blue-100' : color === 'green' ? 'bg-green-100' : 'bg-purple-100')}>
      ...
      {/* Revenue Tab */}
      {tab === 'revenue' && (<div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {[
                ...
                { label: 'Avg Monthly', value: `$${revenue?.length ? Math.round(revenue.reduce((s, r) => s + r.revenue, 0) / revenue.length).toLocaleString() : 0}`, icon: Activity },
            ].map(({ label, value, icon: Icon }) => (<Card key={label}>
          ...
          <Card>
            ...
            <CardContent>
              {revenueLoading ? <ChartSkeleton height={300}/> : (<ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={revenue ?? []}>
                    <defs>
      ...
      {/* AI Tab */}
      {tab === 'ai' && (<div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {[
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <Card>
              ...
              <CardContent>
                {aiLoading ? <ChartSkeleton height={280}/> : (<ResponsiveContainer width="100%" height={280}>
                    <BarChart data={(aiUsage ?? []).slice(0, 8)} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                      <XAxis type="number" tick={{ fontSize: 10 }}/>
                      <YAxis dataKey="module" type="category" tick={{ fontSize: 10 }} width={140} tickFormatter={(m) => m.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}/>
