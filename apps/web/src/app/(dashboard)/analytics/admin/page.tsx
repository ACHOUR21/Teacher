  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {kpis.map(({ label, value, icon: Icon, iconBg, iconColor, sub }) => (
          <Card key={label}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={revenue ?? []}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => [fmt.format(v), 'Revenue']}
                />
                <Area
                  type="monotone"
                  dataKey="revenue"
        ...
        <Card>
          ...
          <CardContent>
            {aiLoading ? (
              ...
              <p className="text-sm text-gray-400 text-center py-16">No AI usage data yet</p>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie
                ...
                <tbody>
                  {(cohort ?? []).map((c: any) => (
                    <tr key={c.week} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      {[c.day1, c.day7, c.day30].map((pct: number, i: number) => (
                        <td key={i} className="py-3 px-4 text-right">
                          <span
                            className="inline-block min-w-[48px] text-center px-2 py-0.5 rounded-md text-xs font-semibold"
                            style={{
