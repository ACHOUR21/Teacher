...
export default function TranscriptPage({ params, }) {
    ...
    const { data: transcript, isLoading, error } = useQuery({
        ...
        queryFn: () => api.get(`/university/transcript/${studentId}`).then((r) => r.data.data ?? r.data),
        enabled: !!studentId,
    });
    if (isLoading) {
    ...
    if (error || !transcript) {
        ...
        <Link href="/university" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700">
    ...
    return (<div className="space-y-6">
      {/* Back nav */}
      <Link href="/university" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 w-fit">
        <ChevronLeft className="h-4 w-4"/>
        Back to University
      </Link>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <img src={student.user.avatarUrl} alt={student.user.firstName} className="h-16 w-16 rounded-full object-cover flex-shrink-0"/>) : (<div className="h-16 w-16 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white text-xl font-bold flex-shrink-0">
              {initials}
            </div>)}
          <div className="flex-1">
            ...
            {student.studentId && (<p className="text-xs text-gray-400 mt-0.5 font-mono">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {enrollments.length === 0 ? (<div className="py-16 text-center">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {enrollments.map((enrollment) => (<tr key={enrollment.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-3.5 px-5 text-center">
                      {enrollment.grade !== null && enrollment.grade !== undefined ? (<span className="font-medium text-gray-900">
                          {enrollment.grade.toFixed(1)}
                        </span>) : (<span className="text-gray-400">—</span>)}
                    </td>
                    <td className="py-3.5 px-5 text-center">
                      {enrollment.letterGrade ? (<span className={cn('inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold', GRADE_COLORS[enrollment.letterGrade] ?? 'bg-gray-100 text-gray-600')}>
