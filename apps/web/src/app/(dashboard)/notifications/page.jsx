...
export default function NotificationsPage() {
    ...
    return (<div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        {unreadCount > 0 && (<Button size="sm" variant="outline" leftIcon={<CheckCheck className="h-4 w-4"/>} onClick={() => markAllRead.mutate()} loading={markAllRead.isPending}>
            Mark all read
          </Button>)}
      </div>

      {notifications.length === 0 ? (<div className="text-center py-20">
          ...
          {orderedGroups.map(groupLabel => (<div key={groupLabel}>
              <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 px-1">
                {groupLabel}
              </h2>
              <div className="bg-card rounded-xl border border-border overflow-hidden divide-y divide-border/50">
                {grouped[groupLabel].map((n) => {
                    const config = TYPE_CONFIG[n.type] ?? TYPE_CONFIG.GENERAL;
                    const Icon = config.icon;
                    const isRead = !!n.isRead;
                    return (<div key={n.id} onClick={() => !isRead && markRead.mutate(n.id)} className={cn('group flex items-start gap-4 px-5 py-4 transition-colors', isRead
