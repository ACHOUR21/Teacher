...
function CouponInput() {
  ...
  const mutation = useMutation({
    mutationFn: (couponCode: string) =>
      api.post('/billing/coupon/apply', { couponCode }).then(r => r.data.data),
    onSuccess: (data) => setResult({ ok: true, message: data.message }),
    onError: (err: any) => setResult({ ok: false, message: err.response?.data?.message ?? 'Invalid coupon' }),
  });
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {result && (
        <p className={cn('text-xs flex items-center gap-1.5', result.ok ? 'text-green-600' : 'text-red-500')}>
      ...
      {analytics && (
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <RevenueCard label="Active Plans" value={analytics.planDistribution.reduce((s: number, p: any) => s + p.count, 0).toString()} icon={Users} color="bg-purple-50 text-purple-600" />
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <button
              ...
              className={cn('relative h-5 w-9 rounded-full transition-colors', annualBilling ? 'bg-blue-600' : 'bg-gray-300')}
            >
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  plan.popular ? 'border-blue-500 shadow-md' : isCurrent ? 'border-green-500' : 'border-gray-200',
                )}
              >
                {plan.popular && (
                    ...
                    {displayPrice !== null ? (
                      displayPrice === 0 ? (
                        ...
                        <>
                          ...
                          {annualBilling && plan.period === 'mo' && (
                            <span className="ml-2 text-xs text-green-600 font-medium">billed annually</span>
                          )}
                        </>
                      )
                    ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!invoices?.invoices?.length && (
                  <tr><td colSpan={5} className="py-8 text-center text-gray-400 text-sm">No invoices yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
