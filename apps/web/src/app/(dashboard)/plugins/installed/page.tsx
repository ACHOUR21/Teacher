      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="p-5 space-y-3">
          <textarea
            ...
            rows={10}
            spellCheck={false}
            className="w-full font-mono text-xs bg-gray-50 border border-gray-200 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
          />
          {error && (
        ...
        <div className="flex justify-end gap-3 px-5 pb-5">
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="px-4 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-60"
          >
...
export default function InstalledPluginsPage() {
  ...
  const [configTarget, setConfigTarget] = useState<InstalledPluginDto | null>(null);
  ...
  const { data: installed, isLoading } = useQuery<InstalledPluginDto[]>({
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ pluginId, enabled }: { pluginId: string; enabled: boolean }) =>
      api.patch(`/plugins/installed/${pluginId}/toggle`, { enabled }),
    onSuccess: () => {
  ...
  const uninstallMutation = useMutation({
    mutationFn: (pluginId: string) => api.delete(`/plugins/installed/${pluginId}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['installed-plugins'] });
  ...
  const configMutation = useMutation({
    mutationFn: ({ pluginId, config }: { pluginId: string; config: Record<string, unknown> }) =>
      api.patch(`/plugins/installed/${pluginId}/config`, { config }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['installed-plugins'] });
    ...
    <div className="space-y-6">
      ...
      {configTarget && (
        <ConfigModal
          ...
          onSave={config => configMutation.mutate({ pluginId: configTarget.pluginId, config })}
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className={cn(
                          'transition-colors',
                          inst.enabled ? 'text-green-500 hover:text-green-600' : 'text-gray-300 hover:text-gray-500',
                        )}
