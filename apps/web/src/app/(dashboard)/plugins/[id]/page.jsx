...
function StarRating({ rating }) {
    return (<div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map(star => (<Star key={star} className={cn('h-4 w-4', star <= Math.round(rating)
...
export default function PluginDetailPage() {
    ...
    return (<div className="max-w-3xl mx-auto space-y-6 pb-12">
      {/* Toast */}
      {toast && (<div className={cn('fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium transition-all', toast.type === 'success'
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className={cn('px-3 py-1 rounded-full text-sm font-semibold', plugin.price === 0
        ...
        {/* Tags */}
        {plugin.tags.length > 0 && (<div className="flex flex-wrap gap-2 mt-4">
            {plugin.tags.map(tag => (<span key={tag} className="text-xs bg-purple-50 text-purple-600 border border-purple-200 px-2 py-0.5 rounded-full">
                #{tag}
              </span>))}
          </div>)}
        ...
        <div className="mt-6 flex gap-3">
          {isInstalled ? (<div className="flex items-center gap-2 px-5 py-2.5 bg-green-50 text-green-700 border border-green-200 rounded-xl text-sm font-medium">
              <CheckCircle className="h-4 w-4"/> Installed
            </div>) : plugin.price > 0 ? (<>
              <button onClick={() => installMutation.mutate()} disabled={installMutation.isPending} className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-xl text-sm font-medium hover:bg-purple-700 transition-colors disabled:opacity-60">
                <Download className="h-4 w-4"/>
                {installMutation.isPending ? 'Installing…' : 'Start Free Trial'}
              </button>
              <button onClick={() => installMutation.mutate()} disabled={installMutation.isPending} className="flex items-center gap-2 px-5 py-2.5 border border-gray-200 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors disabled:opacity-60">
      ...
      {/* Permissions */}
      {plugin.permissions.length > 0 && (<div className="bg-white rounded-2xl border border-gray-200 p-6">
          <h2 className="text-base font-semibold text-gray-900 flex items-center gap-2 mb-4">
            <Shield className="h-5 w-5 text-gray-500"/> Required Permissions
          </h2>

          {hasHighRisk && (<div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl p-3 mb-4 text-sm text-red-700">
          ...
          <div className="space-y-2">
            {plugin.permissions.map(perm => {
                ...
                return (<div key={perm} className={cn('flex items-center justify-between px-3 py-2 rounded-lg border text-sm', RISK_COLORS[risk])}>
                  <span>{perm}</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[1, 2].map(i => (<div key={i} className="aspect-video bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 text-sm">
              Screenshot {i}
            </div>))}
        </div>
      </div>
