...
export default function LivePage() {
  ...
  const createMutation = useMutation({
    mutationFn: (dto: typeof form) => api.post('/live/sessions', dto).then(r => r.data.data),
  ...
  const startMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/live/sessions/${id}/start`).then(r => r.data.data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['live-sessions'] }),
  });

  const upcoming = (sessions?.data ?? []).filter((s: any) => s.status === 'SCHEDULED');
  const live = (sessions?.data ?? []).filter((s: any) => s.status === 'LIVE');
  const past = (sessions?.data ?? []).filter((s: any) => s.status === 'ENDED');
      ...
      {live.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {live.map((s: any) => (
              <div key={s.id} className="bg-red-50 border border-red-200 rounded-xl p-4">
