...
function XpProgressBar({ percent, current, needed, level }) {
    return (<div className="bg-white rounded-xl border border-gray-200 p-4">
      ...
      <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
        <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transition-all duration-500" style={{ width: `${Math.min(percent, 100)}%` }}/>
      </div>
      <p className="text-xs text-gray-400 mt-1.5">{percent}% progress to level {level + 1}</p>
    </div>);
}
function StreakBadge({ streak, longest }) {
    return (<div className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
      <div className={cn('h-12 w-12 rounded-xl flex items-center justify-center flex-shrink-0', streak >= 7 ? 'bg-orange-50' : 'bg-gray-50')}>
      ...
      {streak >= 7 && (<span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-full font-medium">
          🔥 On fire!
        </span>)}
    </div>);
}
function ActivityFeed({ events }) {
    ...
    return (<div className="space-y-2 max-h-72 overflow-y-auto">
      {events.map((ev) => {
            ...
            return (<div key={ev.id} className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-50">
    ...
    return (<div className="space-y-6">
      ...
      {stats && (<div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            ].map(stat => (<div key={stat.label} className="bg-white rounded-xl border border-gray-200 p-4">
      ...
      {/* XP progress + streak */}
      {stats && (<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {xp && (<XpProgressBar percent={xp.percent} current={xp.current} needed={xp.needed} level={xp.level}/>)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {leaderboardData.length === 0 ? (<p className="text-sm text-gray-400 py-4 text-center">No data yet</p>) : (<div className="space-y-2">
              {leaderboardData.slice(0, 10).map((entry, i) => {
                const RankIcon = i < 3 ? RANK_ICONS[i] : null;
                return (<div key={entry.userId ?? i} className={cn('flex items-center gap-3 px-3 py-2.5 rounded-lg', i < 3 ? RANK_BG[i] : 'hover:bg-gray-50')}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {allAchievements.length === 0 ? (<p className="text-sm text-gray-400 py-4 text-center">No achievements configured</p>) : (<div className="space-y-2 max-h-80 overflow-y-auto">
              {allAchievements.map((ach) => {
                const earned = earnedIds.has(ach.id);
                return (<div key={ach.id} className={cn('flex items-center gap-3 p-3 rounded-lg border transition-colors', earned ? 'border-green-200 bg-green-50' : 'border-gray-100 bg-gray-50 opacity-60')}>
