...
export default function StudySessionPage() {
    ...
    const reviewMutation = useMutation({
        mutationFn: ({ cardId, rating }) => api.post(`/flashcards/cards/${cardId}/review`, { rating }),
        onSuccess: () => {
            setReviewed(r => r + 1);
            setIsFlipped(false);
    ...
    if (done || cards.length === 0) {
        ...
        {stats && (<div className="grid grid-cols-3 gap-4 mt-4 text-center">
    ...
    return (<div className="max-w-lg mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="w-full bg-muted rounded-full h-1.5">
            <div className="bg-primary h-1.5 rounded-full transition-all" style={{ width: `${(currentIdx / cards.length) * 100}%` }}/>
          </div>
        </div>
      ...
      {/* Card */}
      <button onClick={() => setIsFlipped(f => !f)} className="w-full min-h-[240px] border-2 border-primary/20 hover:border-primary/50 rounded-2xl p-8 flex flex-col items-center justify-center gap-3 bg-card transition-all cursor-pointer shadow-sm hover:shadow-md">
        ...
        {!isFlipped && card.hint && (<p className="text-xs text-muted-foreground italic mt-2">Hint: {card.hint}</p>)}
      </button>
      ...
      {isFlipped ? (<div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {RATING_LABELS.map(r => (<button key={r.value} onClick={() => reviewMutation.mutate({ cardId: card.id, rating: r.value })} disabled={reviewMutation.isPending} className={cn('py-2.5 rounded-xl text-sm font-medium transition-colors', r.color)}>
                {r.label}
              </button>))}
          </div>
