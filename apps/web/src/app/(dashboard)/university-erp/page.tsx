...
// Types
// ---------------------------------------------------------------------------
interface University { id: string; name: string; code?: string; _count?: { faculties: number; programs: number }; enrollmentsCount?: number; activeStudents?: number }
interface Faculty { id: string; name: string; code?: string; _count?: { departments: number } }
interface UniDepartment { id: string; name: string; code?: string; _count?: { programs: number } }
interface AcademicProgram { id: string; name: string; code?: string; degree: string; durationYears: number; department?: { name: string }; _count?: { enrollments: number } }
interface Enrollment { id: string; studentId: string; programId: string; status: string; startedAt: string; completedAt?: string; student?: { gpa?: number; studentId?: string; user?: { firstName: string; lastName: string; email: string } }; program?: { name: string } }
interface AcademicRecord { id: string; gpa?: number; user: { firstName: string; lastName: string; email: string }; enrollments: { id: string; status: string; startedAt: string; completedAt?: string; program: { name: string; code?: string; degree: string; durationYears: number } }[] }
...
// ---------------------------------------------------------------------------
function EnrollmentStatsPanel({ programId, programName, onClose }: { programId: string; programName: string; onClose: () => void }) {
          ...
          {total > 0 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {items.filter(i => i.value > 0).map(item => (
                <span key={item.label} className={cn('px-3 py-1 rounded-full text-xs font-medium', STATUS_COLORS[item.label.toUpperCase()] ?? 'bg-gray-100 text-gray-600')}>
...
function FacultyCard({
  ...
  const { data: departments } = useQuery<UniDepartment[]>({
      ...
      {expanded && (
        <div className="mt-4 border-t border-gray-100 pt-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              onClick={() => onAddDepartment(faculty.id)}
...
export default function UniversityErpPage() {
  ...
  const [showAddDeptForFacultyId, setShowAddDeptForFacultyId] = useState<string | null>(null);
  const [showAddProgram, setShowAddProgram] = useState(false);
  const [showEnrollStudent, setShowEnrollStudent] = useState(false);
  const [statsProgramId, setStatsProgramId] = useState<string | null>(null);
  ...
  const [selectedStudentUniversityId, setSelectedStudentUniversityId] = useState<string | null>(null);
  ...
  const { data: univDetail } = useQuery<University>({
  ...
  const { data: programs } = useQuery<AcademicProgram[]>({
  ...
  const { data: academicRecord } = useQuery<AcademicRecord>({
  ...
  const addFacultyMutation = useMutation({
    mutationFn: (data: { name: string; code?: string }) =>
      api.post(`/university-erp/universities/${universityId}/faculties`, data).then(r => r.data.data),
    onSuccess: () => {
  ...
  const deleteFacultyMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/university-erp/faculties/${id}`).then(r => r.data.data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['university-faculties', universityId] });
      qc.invalidateQueries({ queryKey: ['university-erp-universities'] });
    },
  ...
  const addDeptMutation = useMutation({
    mutationFn: ({ facultyId, data }: { facultyId: string; data: { name: string; code?: string } }) =>
      api.post(`/university-erp/faculties/${facultyId}/departments`, data).then(r => r.data.data),
    onSuccess: (_, vars) => {
  ...
  const addProgramMutation = useMutation({
    mutationFn: (data: typeof programForm) =>
      api.post(`/university-erp/universities/${universityId}/programs`, {
        ...data,
  ...
  const enrollStudentMutation = useMutation({
    mutationFn: (data: { studentId: string; programId: string }) =>
      api.post(`/university-erp/programs/${data.programId}/enroll`, { studentId: data.studentId }).then(r => r.data.data),
    onSuccess: () => {
  ...
  const updateEnrollStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      api.patch(`/university-erp/enrollments/${id}/status`, { status }).then(r => r.data.data),
    onSuccess: () => {
  ...
  // Computed
  // ---------------------------------------------------------------------------
  const totalFaculties = univDetail?._count?.faculties ?? (universities as University[] | undefined)?.reduce((s, u) => s + (u._count?.faculties ?? 0), 0) ?? 0;
  const totalPrograms = univDetail?._count?.programs ?? (universities as University[] | undefined)?.reduce((s, u) => s + (u._count?.programs ?? 0), 0) ?? 0;
    ...
    <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {faculties.map(faculty => (
                <FacultyCard
                  ...
                  onAddDepartment={(fid) => setShowAddDeptForFacultyId(fid)}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {(!programs || programs.length === 0) ? (
                  <tr><td colSpan={6} className="py-12 text-center text-gray-400">No programs found</td></tr>
                ) : programs.map(prog => (
                  <tr key={prog.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-3.5 px-4">
                      <button
                        onClick={() => { setStatsProgramId(prog.id); setStatsProgramName(prog.name); }}
                        className="flex items-center gap-1 text-xs text-purple-600 hover:text-purple-700 font-medium"
                      >
      ...
      {activeSection === 'enrollments' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <select
              value={enrollFilterProgram}
              onChange={e => { setEnrollFilterProgram(e.target.value); setEnrollPage(1); }}
              className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
            ...
            <select
              value={enrollFilterStatus}
              onChange={e => { setEnrollFilterStatus(e.target.value); setEnrollPage(1); }}
              className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
                      ...
                      return (
                        <tr key={enroll.id} className="hover:bg-gray-50">
                          ...
                          <td className="py-3.5 px-4">
                            <select
                              value={enroll.status}
                              onChange={e => updateEnrollStatusMutation.mutate({ id: enroll.id, status: e.target.value })}
                              className="px-2 py-1 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-2 focus:ring-purple-300"
                            >
                              <option value="ACTIVE">Active</option>
                              <option value="COMPLETED">Completed</option>
                              <option value="SUSPENDED">Suspended</option>
                ...
                <div className="flex items-center justify-between text-sm text-gray-600">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-40 disabled:pointer-events-none"
                    >
                      Prev
                    </button>
                    <button
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-40 disabled:pointer-events-none transition-colors"
              >
                Search
              </button>
            </div>
            ...
            academicRecord ? (
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  ...
                  {(!academicRecord.enrollments || academicRecord.enrollments.length === 0) ? (
                    <div className="py-10 text-center text-gray-400 text-sm">No enrollments found for this university</div>
                  ) : (
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50">
                        <tr>
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {academicRecord.enrollments.map(enroll => (
                          <tr key={enroll.id} className="hover:bg-gray-50">
      ...
      <Modal open={showAddFaculty} onClose={() => setShowAddFaculty(false)} title="Add Faculty">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              onClick={() => addFacultyMutation.mutate({ name: facultyForm.name, code: facultyForm.code || undefined })}
              disabled={!facultyForm.name.trim() || addFacultyMutation.isPending}
      ...
      <Modal open={!!showAddDeptForFacultyId} onClose={() => setShowAddDeptForFacultyId(null)} title="Add Department">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              onClick={() => showAddDeptForFacultyId && addDeptMutation.mutate({ facultyId: showAddDeptForFacultyId, data: { name: deptForm.name, code: deptForm.code || undefined } })}
              disabled={!deptForm.name.trim() || addDeptMutation.isPending}
              className="px-5 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50 disabled:pointer-events-none transition-colors"
            >
      ...
      <Modal open={showAddProgram} onClose={() => setShowAddProgram(false)} title="Add Academic Program">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={e => setProgramForm(f => ({ ...f, degree: e.target.value }))}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={e => setProgramForm(f => ({ ...f, durationYears: Number(e.target.value) }))}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-400"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={e => setProgramForm(f => ({ ...f, departmentId: e.target.value }))}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              onClick={() => addProgramMutation.mutate(programForm)}
              disabled={!programForm.name.trim() || addProgramMutation.isPending}
              className="px-5 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50 disabled:pointer-events-none transition-colors"
            >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setEnrollForm(f => ({ ...f, programId: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-400"
            >
              <option value="">Select Program</option>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              onClick={() => enrollStudentMutation.mutate({ studentId: enrollForm.studentId, programId: enrollForm.programId })}
              disabled={!enrollForm.studentId.trim() || !enrollForm.programId || enrollStudentMutation.isPending}
              className="px-5 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50 disabled:pointer-events-none transition-colors"
            >
      ...
      {statsProgramId && (
        <EnrollmentStatsPanel
          programId={statsProgramId}
          programName={statsProgramName}
          onClose={() => { setStatsProgramId(null); setStatsProgramName(''); }}
        />
      )}
    </div>
  );
