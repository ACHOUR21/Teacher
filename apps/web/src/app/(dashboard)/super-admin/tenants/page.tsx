...
import {
  Building2, Search, CheckCircle, XCircle, Edit2,
  Trash2, LogIn, Eye, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
export default function TenantsManagementPage() {
  ...
  const planMut = useMutation({
    mutationFn: ({ id, plan }: { id: string; plan: string }) =>
      api.put(`/super-admin/tenants/${id}/plan`, { plan }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['sa-tenants-page'] });
  ...
  const impersonateMut = useMutation({
    mutationFn: (id: string) => api.post(`/super-admin/tenants/${id}/impersonate`).then(r => r.data.data),
    onSuccess: (data) => {
      ...
      <div className="bg-card border border-border rounded-xl overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/40">
            <tr>
              {['Tenant', 'Plan', 'Users', 'Status', 'Created', 'Actions'].map(h => (
          ...
          <tbody className="divide-y divide-border">
            ...
            {!isLoading && data?.items?.map((t) => (
              <tr key={t.id as string} className="hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3"><PlanBadge plan={t.plan as string} /></td>
                <td className="px-4 py-3 text-muted-foreground">
                  {(t._count as Record<string, number>)?.users ?? 0}
                </td>
                <td className="px-4 py-3"><StatusBadge active={t.isActive as boolean} /></td>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <Link
                      href={`/super-admin/tenants/${t.id as string}`}
                    ...
                    <button
                      onClick={() => { setEditingTenant(t); setEditPlan(t.plan as string); }}
                      className="p-1.5 hover:bg-accent rounded transition-colors text-muted-foreground hover:text-foreground"
                      title="Edit plan"
                    >
                    ...
                    <button
                      onClick={() => setConfirmAction({
                        type: (t.isActive as boolean) ? 'suspend' : 'activate',
                        tenant: t,
                      })}
                      className={`p-1.5 hover:bg-accent rounded transition-colors ${(t.isActive as boolean) ? 'text-amber-500' : 'text-green-500'}`}
                      title={(t.isActive as boolean) ? 'Suspend tenant' : 'Activate tenant'}
                    >
                      {(t.isActive as boolean) ? <XCircle className="h-3.5 w-3.5" /> : <CheckCircle className="h-3.5 w-3.5" />}
                    </button>

                    <button
                      onClick={() => impersonateMut.mutate(t.id as string)}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-sm shadow-xl">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-semibold text-foreground">Change Plan — {editingTenant.name as string}</h3>
            ...
            <div className="flex justify-end gap-2 p-4 border-t border-border">
              ...
              <button
                onClick={() => planMut.mutate({ id: editingTenant.id as string, plan: editPlan })}
        ...
        <ConfirmModal
          title={
            confirmAction.type === 'delete'
              ? `Delete "${confirmAction.tenant.name as string}"?`
              : confirmAction.type === 'suspend'
              ? `Suspend "${confirmAction.tenant.name as string}"?`
              : `Activate "${confirmAction.tenant.name as string}"?`
          }
          description={
            confirmAction.type === 'delete'
              ...
              : 'This will re-enable access for all users in this tenant.'
          }
          danger={confirmAction.type === 'delete'}
          onCancel={() => setConfirmAction(null)}
          onConfirm={() => {
            if (confirmAction.type === 'delete') {
              deleteMut.mutate(confirmAction.tenant.id as string);
            } else {
              statusMut.mutate({
                id: confirmAction.tenant.id as string,
