...
import { Shield, Building2, Users, TrendingUp, DollarSign, Activity, Search, CheckCircle, XCircle, Zap, LogIn, Trash2, Edit2, X, ExternalLink, Database, Radio, Cpu, } from 'lucide-react';
...
function KpiCard({ icon: Icon, label, value, sub, color = 'text-primary' }) {
    return (<div className="bg-card border border-border rounded-xl p-5 flex items-center gap-4">
...
function HealthDot({ status }) {
    ...
    return (<span className={`inline-flex items-center gap-1.5 text-xs font-medium ${isHealthy ? 'text-green-600 dark:text-green-400' : isUnknown ? 'text-muted-foreground' : 'text-red-500'}`}>
...
function OverviewTab() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <KpiCard icon={Building2} label="Total Tenants" value={stats.totalTenants} sub={`${stats.activeTenants} active`} color="from-blue-500 to-blue-600"/>
        <KpiCard icon={Users} label="Total Users" value={stats.totalUsers.toLocaleString()} sub={`${stats.activeUsers.toLocaleString()} active`} color="from-violet-500 to-violet-600"/>
        <KpiCard icon={DollarSign} label="Total Revenue" value={`$${stats.totalRevenue.toLocaleString()}`} sub={`${stats.totalEnrollments.toLocaleString()} enrollments`} color="from-green-500 to-emerald-600"/>
        <KpiCard icon={TrendingUp} label="Monthly Revenue (MRR)" value={`$${(stats.mrr ?? 0).toLocaleString()}`} sub="Estimated MRR" color="from-amber-500 to-orange-500"/>
        <KpiCard icon={Zap} label="AI Cost Today" value={`$${(stats.aiCostToday ?? 0).toFixed(2)}`} sub="All tenants combined" color="from-rose-500 to-pink-600"/>
        <KpiCard icon={Activity} label="Platform Uptime" value={`${stats.uptimePercent ?? 99.9}%`} sub={`${stats.pendingSupportTickets ?? 0} pending tickets`} color="from-cyan-500 to-cyan-600"/>
      </div>

      {/* System Health Indicators */}
      {health && (<div className="bg-card border border-border rounded-xl p-5">
              ...
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie data={planBreakdown} dataKey="count" nameKey="plan" cx="50%" cy="50%" outerRadius={65}>
                    {planBreakdown.map((entry) => (<Cell key={entry.plan} fill={PLAN_COLORS[entry.plan] ?? '#94a3b8'}/>))}
                  </Pie>
                  <Tooltip formatter={(v, n) => [v, PLAN_LABELS[n] ?? n]}/>
                </PieChart>
              </ResponsiveContainer>
              <div className="space-y-1.5 mt-2">
                {planBreakdown.map((p) => (<div key={p.plan} className="flex items-center justify-between text-xs">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        ].map(({ href, label, sub, icon: Icon, color }) => (<Link key={href} href={href} className="bg-card border border-border rounded-xl p-4 flex items-center gap-3 hover:bg-accent/40 transition-colors group">
      ...
      <div className="bg-card border border-border rounded-xl">
        ...
        <div className="divide-y divide-border">
          {recentTenants.map((t) => (<div key={t.id} className="flex items-center justify-between px-5 py-3 hover:bg-muted/30 transition-colors">
              <div>
          ...
          {recentTenants.length === 0 && (<p className="text-center text-sm text-muted-foreground py-8">No tenants yet</p>)}
        </div>
      </div>
    </div>);
}
...
function TenantsTab() {
    ...
    const updateMut = useMutation({
        mutationFn: ({ id, ...dto }) => api.patch(`/super-admin/tenants/${id}`, dto),
    ...
    return (<div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Search tenants..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') {
        setQ(search);
        setPage(1);
    } }}/>
        </div>
        <button onClick={() => { setQ(search); setPage(1); }} className="px-4 py-2 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors">
          Search
        </button>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-muted/40">
            <tr>
              {['Tenant', 'Type', 'Plan', 'Users', 'Status', 'Created', 'Actions'].map(h => (<th key={h} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">{h}</th>))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading && (<tr><td colSpan={7} className="text-center py-12"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary mx-auto"/></td></tr>)}
            {!isLoading && data?.items?.map((t) => (<tr key={t.id} className="hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => updateMut.mutate({ id: t.id, isActive: !t.isActive })} className={`p-1.5 hover:bg-accent rounded transition-colors ${t.isActive ? 'text-amber-500' : 'text-green-500'}`} title={t.isActive ? 'Suspend' : 'Activate'}>
                      {t.isActive ? <XCircle className="h-3.5 w-3.5"/> : <CheckCircle className="h-3.5 w-3.5"/>}
                    </button>
                    <button onClick={() => { if (confirm(`Delete tenant "${t.name}" and ALL its data?`)) {
            deleteMut.mutate(t.id);
        } }} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors text-red-400 hover:text-red-600" title="Delete">
                      <Trash2 className="h-3.5 w-3.5"/>
                    </button>
                  </div>
                </td>
              </tr>))}
            {!isLoading && !data?.items?.length && (<tr><td colSpan={7} className="text-center py-12 text-muted-foreground">No tenants found</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      {data && data.totalPages > 1 && (<div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-3 py-1.5 text-sm border border-border rounded-md disabled:opacity-40 hover:bg-accent transition-colors">← Prev</button>
            <span className="px-3 py-1.5 text-sm text-muted-foreground">{page} / {data.totalPages}</span>
            <button disabled={page === data.totalPages} onClick={() => setPage(p => p + 1)} className="px-3 py-1.5 text-sm border border-border rounded-md disabled:opacity-40 hover:bg-accent transition-colors">Next →</button>
          </div>
        </div>)}
          ...
          <div className="bg-card border border-border rounded-xl w-full max-w-sm shadow-xl">
            ...
            <div className="p-4 space-y-3">
              <div>
                ...
                <select className="w-full border border-border rounded-md px-3 py-2 text-sm bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary" value={editPlan} onChange={e => setEditPlan(e.target.value)}>
            ...
            <div className="flex justify-end gap-2 p-4 border-t border-border">
              ...
              <button onClick={() => updateMut.mutate({ id: editingTenant.id, plan: editPlan })} disabled={updateMut.isPending} className="px-4 py-2 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors disabled:opacity-50">
...
function UsersTab() {
    ...
    const updateMut = useMutation({
        mutationFn: ({ id, ...dto }) => api.patch(`/super-admin/users/${id}`, dto),
        onSuccess: () => qc.invalidateQueries({ queryKey: ['sa-users'] }),
    });
    const impersonateMut = useMutation({
        mutationFn: (userId) => api.post(`/super-admin/impersonate/${userId}`).then(r => r.data.data),
    ...
    return (<div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
          <input className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Search by name or email..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') {
        setQ(search);
        setPage(1);
    } }}/>
        </div>
        <select className="border border-border rounded-md px-3 py-2 text-sm bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary" value={roleFilter} onChange={e => { setRoleFilter(e.target.value); setPage(1); }}>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-muted/40">
            <tr>
              {['User', 'Role', 'Tenant', 'Status', 'Joined', 'Actions'].map(h => (<th key={h} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">{h}</th>))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading && (<tr><td colSpan={6} className="text-center py-12"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary mx-auto"/></td></tr>)}
            {!isLoading && data?.items?.map((u) => (<tr key={u.id} className="hover:bg-muted/20 transition-colors">
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <button onClick={() => updateMut.mutate({ id: u.id, isActive: !u.isActive })} className={`p-1.5 hover:bg-accent rounded transition-colors ${u.isActive ? 'text-amber-500' : 'text-green-500'}`} title={u.isActive ? 'Suspend user' : 'Activate user'}>
                      {u.isActive ? <XCircle className="h-3.5 w-3.5"/> : <CheckCircle className="h-3.5 w-3.5"/>}
                    </button>
                    <button onClick={() => impersonateMut.mutate(u.id)} className="p-1.5 hover:bg-accent rounded transition-colors text-muted-foreground hover:text-foreground" title="Impersonate user">
                      <LogIn className="h-3.5 w-3.5"/>
                    </button>
                  </div>
                </td>
              </tr>))}
            {!isLoading && !data?.items?.length && (<tr><td colSpan={6} className="text-center py-12 text-muted-foreground">No users found</td></tr>)}
          </tbody>
        </table>
      </div>
      ...
      {impersonateResult && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-lg shadow-xl">
            ...
            <div className="p-4 space-y-3">
              ...
              <button onClick={() => { navigator.clipboard.writeText(impersonateResult.accessToken); }} className="w-full py-2 text-sm border border-border rounded-md hover:bg-accent transition-colors">
                Copy Token
              </button>
            </div>
          </div>
...
function BillingTab() {
    ...
    return (<div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KpiCard icon={DollarSign} label="Total Revenue" value={`$${totalRevenue.toLocaleString()}`} color="from-green-500 to-emerald-600"/>
        <KpiCard icon={Activity} label="Paid Invoices" value={totalInvoices} color="from-blue-500 to-blue-600"/>
        <KpiCard icon={Zap} label="Active Plans" value={planRevenue.reduce((sum, p) => sum + p.count, 0)} color="from-violet-500 to-violet-600"/>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="space-y-2">
            ...
            {planRevenue.map((p) => (<div key={p.plan} className="flex items-center gap-3">
                <span className="w-28 text-xs text-muted-foreground">{PLAN_LABELS[p.plan] ?? p.plan}</span>
                <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{
                width: `${Math.max(4, (p.count / Math.max(...planRevenue.map((x) => x.count))) * 100)}%`,
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="space-y-3">
            ...
            {subscriptionStats.map((s) => (<div key={s.status} className="flex items-center justify-between">
      ...
      <div className="bg-card border border-border rounded-xl">
        ...
        <div className="divide-y divide-border">
          ...
          {recentInvoices.map((inv) => (<div key={inv.id} className="flex items-center justify-between px-5 py-3">
              <div>
...
export default function SuperAdminPage() {
    ...
    return (<div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 border-b border-border">
        {TABS.map(tab => {
            const Icon = tab.icon;
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.id
