...
import { Users, Search, CheckCircle, XCircle, LogIn, Trash2, ChevronLeft, ChevronRight, Shield, } from 'lucide-react';
...
function RoleBadge({ role }) {
    const color = ROLE_COLORS[role] ?? 'bg-muted text-muted-foreground';
    return (<span className={`text-xs font-medium px-2 py-0.5 rounded-full ${color}`}>
      {role.replace(/_/g, ' ')}
    </span>);
}
export default function UsersManagementPage() {
    ...
    const roleMut = useMutation({
        mutationFn: ({ id, role }) => api.put(`/super-admin/users/${id}/role`, { role }),
        onSuccess: () => {
    ...
    const toggleActiveMut = useMutation({
        mutationFn: ({ id, isActive }) => api.patch(`/super-admin/users/${id}`, { isActive }),
        onSuccess: () => qc.invalidateQueries({ queryKey: ['sa-users-page'] }),
    });
    const deleteMut = useMutation({
        mutationFn: (id) => api.delete(`/super-admin/users/${id}`),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['sa-users-page'] });
    ...
    return (<div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[220px] max-w-sm">
          ...
          <input className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-md bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Search by email or name..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') {
        handleSearch();
    } }}/>
        </div>

        <select className="border border-border rounded-md px-3 py-2 text-sm bg-background text-foreground" value={roleFilter} onChange={e => { setRoleFilter(e.target.value); setPage(1); }}>
      ...
      <div className="bg-card border border-border rounded-xl overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/40">
            <tr>
              {['User', 'Role', 'Tenant', 'Last Login', 'Status', 'Actions'].map(h => (<th key={h} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                  {h}
                </th>))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            ...
            {!isLoading && data?.items?.map((u) => (<tr key={u.id} className="hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3">
                  {u.isActive ? (<span className="flex items-center gap-1 text-xs font-medium text-green-600 dark:text-green-400">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    {/* Change Role */}
                    <button onClick={() => setChangeRoleModal({ user: u, newRole: u.role })} className="p-1.5 hover:bg-accent rounded transition-colors text-muted-foreground hover:text-foreground text-xs font-medium" title="Change role">
                      Role
                    </button>

                    {/* Toggle active */}
                    <button onClick={() => toggleActiveMut.mutate({ id: u.id, isActive: !u.isActive })} className={`p-1.5 hover:bg-accent rounded transition-colors ${u.isActive ? 'text-amber-500' : 'text-green-500'}`} title={u.isActive ? 'Deactivate' : 'Activate'}>
                      {u.isActive ? <XCircle className="h-3.5 w-3.5"/> : <CheckCircle className="h-3.5 w-3.5"/>}
                    </button>

                    {/* Impersonate */}
                    <button onClick={() => impersonateMut.mutate(u.id)} disabled={impersonateMut.isPending} className="p-1.5 hover:bg-accent rounded transition-colors text-muted-foreground hover:text-foreground disabled:opacity-50" title="Impersonate user">
                      <LogIn className="h-3.5 w-3.5"/>
                    </button>

                    {/* Delete (GDPR) */}
                    <button onClick={() => setConfirmDelete(u)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors text-red-400 hover:text-red-600" title="Delete user (GDPR)">
      ...
      {/* Change Role Modal */}
      {changeRoleModal && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-sm shadow-xl">
            ...
            <div className="p-4 space-y-2">
              ...
              <select className="w-full border border-border rounded-md px-3 py-2 text-sm bg-background text-foreground" value={changeRoleModal.newRole} onChange={e => setChangeRoleModal(prev => prev ? { ...prev, newRole: e.target.value } : null)}>
                {ROLES.map(r => <option key={r} value={r}>{r.replace(/_/g, ' ')}</option>)}
              </select>
            </div>
            <div className="flex justify-end gap-2 p-4 border-t border-border">
              ...
              <button onClick={() => roleMut.mutate({ id: changeRoleModal.user.id, role: changeRoleModal.newRole })} disabled={roleMut.isPending} className="px-4 py-2 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors disabled:opacity-50">
      ...
      {confirmDelete && (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-sm shadow-xl">
            ...
            <div className="flex justify-end gap-2 p-4 border-t border-border">
              ...
              <button onClick={() => deleteMut.mutate(confirmDelete.id)} disabled={deleteMut.isPending} className="px-4 py-2 text-sm bg-red-600 hover:bg-red-700 text-white rounded-md transition-colors disabled:opacity-50">
