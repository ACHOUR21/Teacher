...
import { Zap, DollarSign, TrendingUp, ChevronLeft, Activity, } from 'lucide-react';
...
function KpiCard({ icon: Icon, label, value, sub, color = 'from-blue-500 to-blue-600', }) {
...
export default function AiUsagePage() {
    ...
    return (<div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={DollarSign} label="AI Cost Today" value={`$${summary.totalCostToday.toFixed(2)}`} color="from-violet-500 to-violet-600"/>
        <KpiCard icon={TrendingUp} label="AI Cost This Month" value={`$${summary.totalCostMonth.toFixed(2)}`} color="from-blue-500 to-blue-600"/>
        <KpiCard icon={DollarSign} label="All-Time AI Cost" value={`$${summary.totalCostAllTime.toFixed(2)}`} color="from-green-500 to-emerald-600"/>
        <KpiCard icon={Activity} label="Requests Today" value={summary.totalRequestsToday.toLocaleString()} color="from-amber-500 to-orange-500"/>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-card border border-border rounded-xl p-5">
          ...
          {topConsumers.length > 0 ? (<ResponsiveContainer width="100%" height={220}>
              <BarChart data={topConsumers} margin={{ top: 0, right: 10, bottom: 30, left: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))"/>
                <XAxis dataKey="tenantName" tick={{ fontSize: 10 }} angle={-30} textAnchor="end" interval={0}/>
            ...
            </ResponsiveContainer>) : (<p className="text-sm text-muted-foreground text-center py-12">No usage data available</p>)}
        </div>
              ...
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie data={modelBreakdown} dataKey="requests" nameKey="model" cx="50%" cy="50%" outerRadius={65}>
                    {modelBreakdown.map((_, i) => (<Cell key={i} fill={MODEL_COLORS[i % MODEL_COLORS.length]}/>))}
                  </Pie>
                  <Tooltip formatter={(v, n) => [v, n]}/>
                </PieChart>
              </ResponsiveContainer>
              <div className="space-y-1.5 mt-2">
                {modelBreakdown.map((m, i) => (<div key={m.model} className="flex items-center justify-between text-xs">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40">
              <tr>
                {['#', 'Tenant', 'Plan', 'Tokens Today', 'Cost Today', 'Tokens This Month', 'Cost This Month'].map(h => (<th key={h} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    {h}
                  </th>))}
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {topConsumers.length === 0 && (<tr>
                  <td colSpan={7} className="text-center py-8 text-muted-foreground">No usage data</td>
                </tr>)}
              {topConsumers.map((t, i) => (<tr key={t.tenantId} className="hover:bg-muted/20 transition-colors">
