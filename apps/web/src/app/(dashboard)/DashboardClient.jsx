    ...
    return (<Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          Live Activity
          {notifications.length > 0 && (<span className="ml-auto text-xs font-normal text-muted-foreground">
              {notifications.length} event{notifications.length !== 1 ? 's' : ''}
            </span>)}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {notifications.length === 0 ? (<div className="flex flex-col items-center justify-center py-8 text-center">
            <Bell className="h-8 w-8 text-muted-foreground/30 mb-2"/>
            <p className="text-sm text-muted-foreground">No recent activity</p>
            <p className="text-xs text-muted-foreground/60 mt-1">
              Events will appear here in real time
            </p>
          </div>) : (<div className="space-y-3 max-h-72 overflow-y-auto scrollbar-thin pr-1">
            {notifications.slice(0, 8).map((n) => (<div key={n.id} className={cn('flex items-start gap-3 text-sm rounded-lg p-2 transition-colors', !n.read && 'bg-primary/5')}>
        ...
        {notifications.length > 0 && (<div className="pt-3 border-t border-border mt-3">
...
function SetupBanner() {
    ...
    if (!isAdmin || dismissed || !onboarding || onboarding.completed) {
        return null;
    }
    const done = (onboarding.completedSteps?.length ?? 0);
    const total = 3;
    const pct = Math.round((done / total) * 100);
    return (<div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl px-5 py-4 flex items-center gap-4 text-white shadow-sm">
      ...
      <Link href="/onboarding" className="flex items-center gap-1 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-lg text-sm font-medium transition-colors shrink-0">
    ...
    return (<Card>
      <CardContent className="py-4">
        <div className="flex items-center gap-4">
          ...
          <button onClick={copy} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium bg-muted hover:bg-accent transition-colors shrink-0">
...
function TeacherDashboard() {
    ...
    const upcoming = (sessions?.data ?? []).filter((s) => s.status === 'SCHEDULED').slice(0, 3);
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        ].map(stat => (<div key={stat.label} className="bg-card rounded-xl border border-border p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card>
          ...
          <CardContent>
            {!profile?.courses?.length ? (<div className="text-center py-6 text-muted-foreground text-sm">
                No published courses yet.{' '}
                <Link href="/courses/new" className="text-primary hover:underline">Create one</Link>
              </div>) : (<div className="space-y-3">
                {profile.courses.map((c) => (<Link key={c.id} href={`/courses/${c.id}`} className="flex items-center gap-3 hover:bg-accent p-2 rounded-lg transition-colors">
        ...
        <Card>
          ...
          <CardContent>
            {!upcoming.length ? (<div className="text-center py-6 text-muted-foreground text-sm">
                No upcoming sessions.{' '}
                <Link href="/live" className="text-primary hover:underline">Schedule one</Link>
              </div>) : (<div className="space-y-3">
                {upcoming.map((s) => (<div key={s.id} className="flex items-center gap-3 p-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          ...
          {isLoading ? (<div className="space-y-3">
            ...
            </div>) : inProgress.length === 0 ? (<div className="bg-card rounded-xl border border-border py-10 text-center">
              ...
              {inProgress.map((p) => (<Link key={p.id} href={`/courses/${p.courseId}/learn`} className="flex items-center gap-4 bg-card rounded-xl border border-border p-4 hover:shadow-sm hover:border-primary/30 transition-all">
                  <div className="h-14 w-20 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex-shrink-0 overflow-hidden">
                    {p.course?.thumbnailUrl && (<img src={p.course.thumbnailUrl} alt="" className="w-full h-full object-cover"/>)}
                  </div>
                  <div className="flex-1 min-w-0">
        ...
        ].map(link => (<Link key={link.href} href={link.href} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-accent transition-colors">
...
export default function DashboardClient() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <StatsCard title="Total Students" value={overview?.totalUsers?.toLocaleString() ?? '—'} icon={Users} trend={{ value: 12.5, direction: 'up' }} color="blue"/>
        <StatsCard title="Active Courses" value={overview?.totalCourses?.toLocaleString() ?? '—'} icon={BookOpen} trend={{ value: 4.1, direction: 'up' }} color="purple"/>
        <StatsCard title="Live Sessions" value={overview?.activeSessions?.toString() ?? '0'} icon={Video} color="green"/>
        <StatsCard title="Monthly Revenue" value={overview?.totalRevenue ? `$${Number(overview.totalRevenue).toLocaleString()}` : '—'} icon={DollarSign} trend={{ value: 8.3, direction: 'up' }} color="orange"/>
      </div>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(topCourses ?? []).map((course, i) => (<tr key={course.id} className="border-b border-border hover:bg-muted/50">
                ...
                {!topCourses?.length && (<tr><td colSpan={4} className="py-8 text-center text-muted-foreground">No courses yet</td></tr>)}
              </tbody>
            </table>
          </div>
        </CardContent>
