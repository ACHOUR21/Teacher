...
import { Download, Search, BookOpen, Users, CheckCircle, BarChart2, } from 'lucide-react';
      ...
      <table className="min-w-full">
        <thead>
          <tr className="border-b border-border bg-muted/30">
            {Array.from({ length: 6 }).map((_, i) => (<th key={i} className="px-4 py-3">
                <div className="h-3 rounded bg-muted animate-pulse w-20"/>
              </th>))}
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: 5 }).map((_, r) => (<tr key={r} className="border-b border-border last:border-0">
              {Array.from({ length: 6 }).map((_, c) => (<td key={c} className="px-4 py-3">
...
function SummaryCard({ icon: Icon, label, value, iconBg, iconColor }) {
    return (<div className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
...
export default function GradeBookPage() {
    ...
    const submissionQueries = useQueries({
        queries: assignments.map((a) => ({
            queryKey: ['subs', a.id],
            queryFn: () => api.get(`/assignments/${a.id}/submissions`).then((r) => r.data.data ?? []),
    ...
    const studentMap = useMemo(() => {
        ...
        assignments.forEach((a, idx) => {
            const subs = submissionQueries[idx]?.data ?? [];
            subs.forEach((sub) => {
    ...
    // ── Per-student average ───────────────────────────────────────────────────
    function getStudentAverage(subs) {
        const graded = assignments.filter((a) => subs[a.id]?.score !== null && subs[a.id]?.score !== undefined);
        if (graded.length === 0) {
            return '—';
        }
        const avg = graded.reduce((sum, a) => sum + (subs[a.id].score / a.maxScore) * 100, 0) /
            graded.length;
    ...
    return (<div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <SummaryCard icon={BookOpen} label="Total Assignments" value={assignments.length} iconBg="bg-indigo-100 dark:bg-indigo-900/30" iconColor="text-indigo-600 dark:text-indigo-400"/>
        <SummaryCard icon={Users} label="Total Submissions" value={totalSubmissions} iconBg="bg-blue-100 dark:bg-blue-900/30" iconColor="text-blue-600 dark:text-blue-400"/>
        <SummaryCard icon={BarChart2} label="Class Average" value={classAverage !== null ? `${classAverage}%` : '—'} iconBg="bg-violet-100 dark:bg-violet-900/30" iconColor="text-violet-600 dark:text-violet-400"/>
        <SummaryCard icon={CheckCircle} label="Graded" value={gradedCount} iconBg="bg-green-100 dark:bg-green-900/30" iconColor="text-green-600 dark:text-green-400"/>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Search students..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 text-sm border border-border rounded-md bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"/>
        </div>
        <button onClick={handleExport} disabled={isLoading || assignments.length === 0} className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0">
      ...
      {/* ── Empty State ── */}
      {!isLoading && assignments.length === 0 && (<div className="flex flex-col items-center justify-center py-24 gap-4 bg-card border border-border rounded-xl">
          <div className="h-14 w-14 rounded-full bg-muted flex items-center justify-center">
            <BookOpen className="h-7 w-7 text-muted-foreground"/>
          </div>
          <p className="text-muted-foreground text-sm text-center">
            No assignments yet.{' '}
            <a href="/assignments" className="text-primary underline underline-offset-2 hover:text-primary/80">
      ...
      {/* ── Grade Table ── */}
      {!isLoading && assignments.length > 0 && (<div className="overflow-x-auto rounded-xl border border-border bg-card shadow-sm">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                {assignments.map((a) => (<th key={a.id} title={a.title} className="px-4 py-3 text-center font-semibold text-foreground whitespace-nowrap min-w-[110px]">
                    {truncate(a.title, 15)}
            ...
            <tbody className="divide-y divide-border">
              {filteredStudents.length === 0 && (<tr>
                  <td colSpan={assignments.length + 2} className="px-4 py-10 text-center text-muted-foreground">
              ...
              {filteredStudents.map((studentName) => {
                ...
                const avg = getStudentAverage(subs);
                return (<tr key={studentName} className="hover:bg-muted/20 transition-colors">
                    ...
                    {assignments.map((a) => {
                        const sub = subs[a.id];
                        if (!sub) {
                            return (<td key={a.id} className="px-4 py-3 text-center text-muted-foreground">
                            —
                          </td>);
                        }
                        if (sub.score !== null && sub.score !== undefined) {
                            return (<td key={a.id} className="px-4 py-3 text-center">
                            <span className={`inline-block px-2 py-0.5 rounded-md text-xs font-semibold ${scoreColor(sub.score, a.maxScore)}`}>
                    ...
                    <td className="px-4 py-3 text-center">
                      {avg === '—' ? (<span className="text-muted-foreground">—</span>) : (<span className={`inline-block px-2 py-0.5 rounded-md text-xs font-bold ${parseInt(avg) >= 70
          ...
          {/* Row count footer */}
          {filteredStudents.length > 0 && (<div className="px-4 py-2 border-t border-border bg-muted/20 text-xs text-muted-foreground text-right">
