...
interface AttemptResult {
  ...
  maxScore: number;
  submittedAt: string;
  exam: { questions: (ExamQuestion & { correctAnswer: string; explanation?: string })[] };
}
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          {result.exam.questions.map((q, i) => {
            ...
            return (
              <div key={q.id} className={cn('border rounded-xl p-4', isCorrect ? 'border-green-200 bg-green-50/50' : 'border-red-200 bg-red-50/50')}>
        ...
        {question.type === 'multiple_choice' && question.options ? (
          <div className="space-y-2">
            {question.options.map((opt, oi) => (
              <button key={oi} onClick={() => setAnswers(prev => ({ ...prev, [question.id]: opt }))}
                className={cn('w-full text-left px-4 py-3 rounded-xl border text-sm transition-colors',
                  answers[question.id] === opt ? 'border-primary bg-primary/5 text-primary font-medium' : 'border-border hover:bg-accent')}>
          ...
          <div className="flex gap-3">
            {['True', 'False'].map(opt => (
              <button key={opt} onClick={() => setAnswers(prev => ({ ...prev, [question.id]: opt.toLowerCase() }))}
                className={cn('flex-1 py-3 rounded-xl border text-sm font-medium transition-colors',
                  answers[question.id] === opt.toLowerCase() ? 'border-primary bg-primary/5 text-primary' : 'border-border hover:bg-accent')}>
                {opt}
              </button>
            ))}
          </div>
