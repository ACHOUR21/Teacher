...
export function ChildCard({ child, isSelected, onClick }: ChildCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'w-full text-left rounded-2xl border p-5 transition-all group',
        isSelected
    ...
    >
      ...
      {/* School */}
      {child.school && (
        <p className="text-xs text-gray-500 mb-3 flex items-center gap-1.5 truncate">
      ...
      <div className="grid grid-cols-3 gap-2">
        <StatMini
          ...
          label="Courses"
          value={String(child.activeCourses)}
          color="blue"
        />
        <StatMini
        ...
        <StatMini
          ...
          label="Streak"
          value={child.currentStreak > 0 ? `${child.currentStreak}d` : '—'}
          color="orange"
        />
      </div>
    </button>
...
function StatMini({
  ...
  return (
    <div className={cn('rounded-lg px-2.5 py-2 text-center', colorMap[color])}>
