    ...
    return (<div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Swap button */}
          <button onClick={swapLanguages} disabled={sourceLang === 'auto'} title={sourceLang === 'auto' ? 'Cannot swap with auto-detect' : 'Swap languages'} className={cn('mt-5 p-2 rounded-lg border border-border transition-colors', sourceLang === 'auto'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <textarea value={sourceText} onChange={e => {
            setSourceText(e.target.value);
            setTranslationResult(null);
        }} rows={8} placeholder="Enter text to translate..." className="w-full px-3 py-2 border border-border rounded-xl text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {translationResult && (<div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground">
                    {Math.round(translationResult.confidence * 100)}% confidence
                  </span>
                  <button onClick={copyTranslation} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
            ...
            <div className="relative">
              <div className={cn('w-full min-h-[176px] px-3 py-2 border rounded-xl text-sm bg-primary/5 border-primary/20', !translationResult && 'text-muted-foreground')}>
                {translateMutation.isPending ? (<div className="flex items-center justify-center h-32 gap-2 text-muted-foreground">
                    <Loader2 className="h-5 w-5 animate-spin"/>
                    <span>Translating…</span>
                  </div>) : translationResult ? (<p className="leading-relaxed whitespace-pre-wrap">
                    {translationResult.translatedText}
                  </p>) : (<p className="italic">Translation will appear here…</p>)}
              </div>
            </div>
            {translationResult?.sourceLanguage && sourceLang === 'auto' && (<p className="text-xs text-muted-foreground">
                Detected language:{' '}
        ...
        {translateMutation.isError && (<p className="text-sm text-destructive">Translation failed. Please try again.</p>)}

        <Button onClick={() => translateMutation.mutate()} disabled={!sourceText.trim() || translateMutation.isPending} className="w-full">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="sm:col-span-2">
            ...
            <input value={courseId} onChange={e => {
            setCourseId(e.target.value);
            setCourseResult(null);
        }} placeholder="e.g. clxyz1234abcdef" className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">
              Target Language
            </label>
            <select value={courseLang} onChange={e => {
        ...
        <Button onClick={() => courseMutation.mutate()} disabled={!courseId.trim() || courseMutation.isPending} variant="outline" className="w-full">
        ...
        {courseMutation.isError && (<p className="text-sm text-destructive">
            Course translation failed. Check the Course ID and try again.
          </p>)}

        {courseResult && (<div className="border border-primary/20 rounded-xl overflow-hidden">
