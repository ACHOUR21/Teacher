...
import { BookOpen, Search, Mic, Volume2, Briefcase, TrendingUp, AlertTriangle, ChevronRight, Loader2, CheckCircle, Download, Brain, FileQuestion, GraduationCap, Layers, Network, Languages, SearchCheck, ShieldCheck, } from 'lucide-react';
...
// ─── Shared Error Banner ─────────────────────────────────────────────────────
function AiErrorBanner({ error }) {
    const apiErr = error;
    const isKeyMissing = apiErr?.statusCode === 500 ||
        apiErr?.statusCode === 0 ||
    ...
    if (isKeyMissing) {
        return (<div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
    ...
    return (<div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. Algebra, World History, Biology..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
        </div>
        <div>
          ...
          <select value={gradeLevel} onChange={e => setGradeLevel(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary">
            {['Grade 6', 'Grade 7', 'Grade 8', 'Grade 9', 'Grade 10', 'Grade 11', 'Grade 12', 'Undergraduate', 'Graduate'].map(g => (<option key={g}>{g}</option>))}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Duration (weeks)</label>
          <input type="number" min={2} max={52} value={weeks} onChange={e => setWeeks(+e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
        </div>
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Learning Objectives (one per line)</label>
          <textarea value={objectives} onChange={e => setObjectives(e.target.value)} rows={3} placeholder="Understand quadratic equations&#10;Apply the Pythagorean theorem&#10;..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
        </div>
      </div>
      ...
      {result && (<div className="border border-border rounded-xl overflow-hidden">
          <div className="bg-primary/5 px-4 py-3 flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={() => {
                ...
                a.download = `curriculum-${result.subject}.json`;
                a.click();
            }}>
          ...
          <div className="divide-y divide-border max-h-[480px] overflow-y-auto">
            {result.weeks?.map((w) => (<div key={w.week} className="p-4">
                ...
                <div className="grid grid-cols-2 gap-3 ml-8">
                  <div>
                    ...
                    <ul className="space-y-0.5">{w.topics?.map((t, i) => <li key={i} className="text-xs text-foreground flex gap-1"><ChevronRight className="h-3 w-3 shrink-0 mt-0.5 text-primary"/>{t}</li>)}</ul>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-muted-foreground mb-1">Activities</p>
                    <ul className="space-y-0.5">{w.activities?.map((a, i) => <li key={i} className="text-xs text-foreground flex gap-1"><CheckCircle className="h-3 w-3 shrink-0 mt-0.5 text-green-500"/>{a}</li>)}</ul>
                  </div>
                </div>
...
function ResearchTab() {
    ...
    return (<div className="space-y-4">
      <div>
        ...
        <textarea value={topic} onChange={e => setTopic(e.target.value)} rows={3} placeholder="Enter a research topic or question..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
      </div>
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">Research Depth</label>
        <div className="grid grid-cols-3 gap-2">
          {['overview', 'detailed', 'academic'].map(d => (<button key={d} onClick={() => setDepth(d)} className={cn('px-3 py-2 rounded-lg text-sm font-medium border transition-colors capitalize', depth === d ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:bg-accent')}>
              {d}
            </button>))}
        </div>
      </div>
      ...
      {mutation.isError && <AiErrorBanner error={mutation.error}/>}
      {result && (<div className="border border-border rounded-xl p-4 bg-muted/30 max-h-[480px] overflow-y-auto">
...
function SpeechToTextTab() {
    ...
    return (<div className="space-y-4">
      <div>
        ...
        <select value={language} onChange={e => setLanguage(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary">
      ...
      <div className="border-2 border-dashed border-border rounded-xl p-8 text-center cursor-pointer hover:border-primary transition-colors" onClick={() => document.getElementById('audio-upload')?.click()}>
      ...
      {mutation.isError && <AiErrorBanner error={mutation.error}/>}
      {result && (<div className="border border-border rounded-xl p-4 bg-muted/30">
...
function TextToSpeechTab() {
    ...
    return (<div className="space-y-4">
      <div>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} rows={5} placeholder="Enter the text you want to convert to speech..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {VOICES.map(v => (<button key={v} onClick={() => setVoice(v)} className={cn('px-3 py-2 rounded-lg text-sm font-medium border transition-colors capitalize', voice === v ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:bg-accent')}>
              {v}
            </button>))}
        </div>
      </div>
      ...
      {mutation.isError && <AiErrorBanner error={mutation.error}/>}
      {audioUrl && (<div className="border border-border rounded-xl p-4 bg-muted/30">
      ...
      {result && (<div className="space-y-4">
          <div className="border border-border rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {result.careerPaths?.map((p, i) => (<div key={i} className="p-3 flex items-start gap-3">
          ...
          {result.skillGaps?.length > 0 && (<div className="border border-border rounded-xl p-3">
              <h3 className="text-sm font-semibold text-foreground mb-2">Skill Gaps to Address</h3>
              <div className="flex flex-wrap gap-1.5">{result.skillGaps.map((s, i) => <span key={i} className="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">{s}</span>)}</div>
            </div>)}

          {result.certifications?.length > 0 && (<div className="border border-border rounded-xl p-3">
              <h3 className="text-sm font-semibold text-foreground mb-2">Recommended Certifications</h3>
              <div className="space-y-2">
                {result.certifications.map((c, i) => (<div key={i} className="flex items-start gap-2">
...
function PerformancePredictionTab() {
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex gap-3">
        <input value={studentId} onChange={e => setStudentId(e.target.value)} placeholder="Student ID (UUID)..." className="flex-1 px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
      ...
      {result && (<div className="border border-border rounded-xl overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            ...
            {result.prediction.recommendations?.length > 0 && (<div>
                <p className="text-sm font-medium text-foreground mb-2">Recommendations</p>
                <ul className="space-y-1">{result.prediction.recommendations.map((r, i) => <li key={i} className="text-sm text-muted-foreground flex gap-2"><ChevronRight className="h-4 w-4 text-primary shrink-0 mt-0.5"/>{r}</li>)}</ul>
              </div>)}
          </div>
        </div>)}
    </div>);
...
function DropoutRiskTab() {
    ...
    return (<div className="space-y-4">
      ...
      {mutation.isError && <AiErrorBanner error={mutation.error}/>}
      {result && (<div className={cn('border rounded-xl overflow-hidden', riskColors[result.riskLevel])}>
          ...
          <div className="p-4 bg-background space-y-3">
            ...
            {result.factors?.length > 0 && (<div>
                <p className="text-sm font-medium text-foreground mb-1">Risk Factors</p>
                <ul className="space-y-1">{result.factors.map((f, i) => <li key={i} className="text-sm text-muted-foreground flex gap-2"><AlertTriangle className="h-4 w-4 text-orange-500 shrink-0 mt-0.5"/>{f}</li>)}</ul>
              </div>)}
            {result.interventions?.length > 0 && (<div>
                <p className="text-sm font-medium text-foreground mb-1">Recommended Interventions</p>
                <ul className="space-y-1">{result.interventions.map((v, i) => <li key={i} className="text-sm text-muted-foreground flex gap-2"><CheckCircle className="h-4 w-4 text-green-500 shrink-0 mt-0.5"/>{v}</li>)}</ul>
              </div>)}
          </div>
        </div>)}
    </div>);
...
function HomeworkTab() {
    ...
    return (<div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">Subject</label>
        <select value={subject} onChange={e => setSubject(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary">
          {SUBJECTS.map(s => <option key={s}>{s}</option>)}
        </select>
      </div>
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">Problem</label>
        <textarea value={problem} onChange={e => setProblem(e.target.value)} rows={4} placeholder="Describe the homework problem in detail..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
      </div>
      ...
      {result && (<div className="space-y-4">
          {result.steps && result.steps.length > 0 && (<div className="border border-border rounded-xl overflow-hidden">
              <div className="bg-primary/5 px-4 py-2">
                <h3 className="text-sm font-semibold text-foreground">Step-by-Step Solution</h3>
              </div>
              <ol className="divide-y divide-border">
                {result.steps.map((step, i) => (<li key={i} className="p-3 flex gap-3 items-start">
          ...
          {result.answer && (<div className="bg-green-50 border border-green-200 rounded-xl p-4">
          ...
          {result.explanation && (<div className="border border-border rounded-xl p-4 bg-muted/30">
              <p className="text-xs font-semibold text-muted-foreground uppercase mb-1">Explanation</p>
              <p className="text-sm text-foreground leading-relaxed">{result.explanation}</p>
            </div>)}
          {!result.steps && !result.answer && (<div className="border border-border rounded-xl p-4 bg-muted/30">
              <p className="text-sm text-foreground leading-relaxed whitespace-pre-wrap">{result.solution ?? JSON.stringify(result)}</p>
            </div>)}
        </div>)}
    </div>);
}
function ExamGeneratorTab() {
    ...
    const toggleType = (value) => setQuestionTypes(prev => prev.includes(value) ? prev.filter(t => t !== value) : [...prev, value]);
    const copyExam = () => {
        if (!result) {
            return;
        }
    ...
    return (<div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. Photosynthesis, World War II..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
        </div>
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Grade Level</label>
          <select value={gradeLevel} onChange={e => setGradeLevel(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {['Easy', 'Medium', 'Hard'].map(d => (<button key={d} onClick={() => setDifficulty(d)} className={cn('px-3 py-2 rounded-lg text-sm font-medium border transition-colors', difficulty === d ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:bg-accent')}>
                {d}
              </button>))}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {QUESTION_TYPES.map(qt => (<button key={qt.value} onClick={() => toggleType(qt.value)} className={cn('px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors', questionTypes.includes(qt.value) ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:bg-accent')}>
              {qt.label}
            </button>))}
        </div>
      </div>
      ...
      {result && (<div className="border border-border rounded-xl overflow-hidden">
          <div className="bg-primary/5 px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {savedExamId ? (<Button variant="outline" size="sm" className="text-green-600 border-green-400" disabled>Saved!</Button>) : (<Button size="sm" onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>
          ...
          <ol className="divide-y divide-border max-h-[480px] overflow-y-auto">
            {result.questions?.map((q, i) => (<li key={i} className="p-4">
                ...
                {q.options && (<div className="ml-6 space-y-1">
                    {q.options.map((opt, oi) => (<div key={oi} className="flex gap-2 text-sm text-foreground">
...
function LessonPlannerTab() {
    ...
    return (<div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Topic</label>
          <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. Introduction to Fractions..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
        </div>
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Grade Level</label>
          <select value={gradeLevel} onChange={e => setGradeLevel(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Learning Objectives (one per line)</label>
          <textarea value={objectives} onChange={e => setObjectives(e.target.value)} rows={3} placeholder="Students will be able to..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
        </div>
      </div>
      ...
      {result && (<div className="border border-border rounded-xl overflow-hidden">
          ...
          <div className="p-4 space-y-4 max-h-[480px] overflow-y-auto">
            {result.objectives?.length > 0 && (<div>
                <h4 className="text-xs font-semibold text-muted-foreground uppercase mb-2">Objectives</h4>
                <ul className="space-y-1">{result.objectives.map((o, i) => <li key={i} className="text-sm text-foreground flex gap-2"><CheckCircle className="h-4 w-4 text-green-500 shrink-0 mt-0.5"/>{o}</li>)}</ul>
              </div>)}
            {result.materials?.length > 0 && (<div>
                <h4 className="text-xs font-semibold text-muted-foreground uppercase mb-2">Materials</h4>
                <div className="flex flex-wrap gap-1.5">{result.materials.map((m, i) => <span key={i} className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{m}</span>)}</div>
              </div>)}
            {result.introduction && (<div>
            ...
            {result.mainContent?.sections?.map((s, i) => (<div key={i} className="border border-border rounded-lg p-3">
            ...
            {result.teacherNotes && (<div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
...
function FlashcardsTab() {
    ...
    return (<div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-foreground mb-1">Topic</label>
          <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. Periodic Table, French Revolution..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
        </div>
        <div>
      ...
      {cards.length > 0 && card && (<div className="space-y-4">
          <div className="flex items-center justify-between text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              ...
              {savedDeckId ? (<span className="text-xs text-green-600 font-medium">Saved to library!</span>) : (<button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending} className="text-xs text-primary font-medium hover:underline disabled:opacity-50">
                  {saveMutation.isPending ? 'Saving...' : 'Save Deck'}
                </button>)}
            </div>
          </div>
          <button onClick={() => setFlippedIndex(isFlipped ? null : currentIndex)} className="w-full min-h-[160px] border-2 border-primary/30 rounded-2xl p-6 text-center transition-all hover:border-primary hover:shadow-md bg-card cursor-pointer">
            {isFlipped ? (<div>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex gap-1">
              {cards.map((_, i) => (<button key={i} onClick={() => { setCurrentIndex(i); setFlippedIndex(null); }} className={cn('w-2 h-2 rounded-full transition-colors', i === currentIndex ? 'bg-primary' : 'bg-border hover:bg-muted-foreground')}/>))}
            </div>
...
function MindMapTab() {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            const branches = (mm.branches ?? []).map((b) => ({
                label: b.label,
                children: (b.children ?? []).map((c) => (typeof c === 'string' ? c : c.label)),
            }));
            setResult({ topic: mm.central ?? topic, central: mm.central ?? topic, branches });
            setExpandedBranches(new Set(branches.map((_, i) => i)));
        },
    });
    const toggleBranch = (i) => setExpandedBranches(prev => {
    ...
    return (<div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">Topic</label>
        <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. Climate Change, Machine Learning..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary"/>
      </div>
      <Button onClick={() => mutation.mutate()} disabled={!topic.trim() || mutation.isPending} className="w-full">
      ...
      {result && (<div className="border border-border rounded-xl overflow-hidden">
          ...
          <div className="p-4 space-y-3 max-h-[480px] overflow-y-auto">
            {result.branches.map((branch, i) => (<div key={i} className={cn('border rounded-lg overflow-hidden', BRANCH_COLORS[i % BRANCH_COLORS.length])}>
                <button onClick={() => toggleBranch(i)} className="w-full flex items-center justify-between px-4 py-2.5 font-medium text-sm">
                  <span>{branch.label}</span>
                  <ChevronRight className={cn('h-4 w-4 transition-transform', expandedBranches.has(i) ? 'rotate-90' : '')}/>
                </button>
                {expandedBranches.has(i) && branch.children.length > 0 && (<div className="px-4 pb-3 pt-1 bg-white/60 space-y-1">
                    {branch.children.map((child, j) => (<div key={j} className="flex items-start gap-2 text-sm text-foreground">
...
function TranslatorTab() {
    ...
    return (<div className="space-y-4">
      ...
      <div>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} rows={5} placeholder="Enter text to translate..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
      </div>
      <Button onClick={() => mutation.mutate()} disabled={!text.trim() || mutation.isPending} className="w-full">
...
function PlagiarismTab() {
    ...
    return (<div className="space-y-4">
      <div>
        ...
        <textarea value={content} onChange={e => setContent(e.target.value)} rows={8} placeholder="Paste the text you want to check for plagiarism..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
      ...
      {result && (<div className="space-y-4">
          <div className="border border-border rounded-xl p-4">
            ...
            {result.verdict && (<span className={cn('text-xs font-semibold px-2 py-1 rounded-full', score < 20 ? 'bg-green-100 text-green-700' : score < 50 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700')}>
                {result.verdict}
              </span>)}
            {result.summary && <p className="text-sm text-muted-foreground mt-3">{result.summary}</p>}
            {result.aiGenerated !== undefined && (<p className="text-sm text-muted-foreground mt-1">AI-generated content: <span className="font-medium text-foreground">{result.aiGenerated ? 'Likely' : 'Unlikely'}</span></p>)}
          </div>

          {result.flags?.length > 0 && (<div className="border border-border rounded-xl p-4">
              <h3 className="text-sm font-semibold text-foreground mb-2">Flags</h3>
              <ul className="space-y-1">
                {result.flags.map((f, i) => (<li key={i} className="flex gap-2 text-sm text-muted-foreground"><AlertTriangle className="h-4 w-4 text-orange-500 shrink-0 mt-0.5"/>{f}</li>))}
              </ul>
            </div>)}

          {result.matches?.length > 0 && (<div className="border border-border rounded-xl overflow-hidden">
              <div className="bg-primary/5 px-4 py-2">
                <h3 className="text-sm font-semibold text-foreground">Matches ({result.matches.length})</h3>
              </div>
              <div className="divide-y divide-border">
                {result.matches.map((m, i) => (<div key={i} className="p-3 space-y-1">
...
function ContentModerationTab() {
    ...
    return (<div className="space-y-4">
      <div>
        ...
        <textarea value={content} onChange={e => setContent(e.target.value)} rows={6} placeholder="Enter content to check for inappropriate material..." className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary resize-none"/>
      </div>
      <Button onClick={() => mutation.mutate()} disabled={!content.trim() || mutation.isPending} className="w-full">
      ...
      {result && (<div className="space-y-4">
          ...
          {result.categories && Object.keys(result.categories).length > 0 && (<div className="border border-border rounded-xl overflow-hidden">
              <div className="bg-primary/5 px-4 py-2">
              ...
              <div className="divide-y divide-border">
                {Object.entries(result.categories).map(([cat, flagged]) => {
                    ...
                    return (<div key={cat} className="p-3 flex items-center gap-3">
                      <div className="w-32 shrink-0">
                        <p className="text-xs font-medium text-foreground capitalize">{cat.replace(/_/g, ' ')}</p>
                      </div>
                      <div className="flex-1 bg-muted rounded-full h-2">
                        <div className={cn('h-2 rounded-full transition-all', (flagged) ? 'bg-red-500' : 'bg-green-400')} style={{ width: `${Math.max(2, pct)}%` }}/>
                      </div>
          ...
          {result.flaggedCategories && result.flaggedCategories.length > 0 && (<div className="border border-red-200 rounded-xl p-4 bg-red-50">
              <h3 className="text-sm font-semibold text-red-700 mb-2">Flagged Issues</h3>
              <ul className="space-y-1">
                {result.flaggedCategories.map((f, i) => (<li key={i} className="flex gap-2 text-sm text-red-700">
...
export default function AIToolsPage() {
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-5 gap-2 sm:grid-cols-8 lg:grid-cols-15">
        {TABS.map(tab => {
            const Icon = tab.icon;
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('flex flex-col items-center gap-1.5 p-3 rounded-xl border text-center transition-all', activeTab === tab.id
      ...
      <div className="bg-card border border-border rounded-2xl p-6">
        <div className="mb-4">
          {TABS.filter(t => t.id === activeTab).map(tab => (<div key={tab.id}>
