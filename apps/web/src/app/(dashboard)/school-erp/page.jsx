...
export default function SchoolErpPage() {
    ...
    const { data: schools } = useQuery({
        ...
        queryFn: () => api.get('/school-erp/schools').then(r => {
            ...
            if (data?.length && !selectedSchoolId) {
    ...
    const handleAssignStudent = async () => {
        if (!assignStudentId.trim() || !classId) {
            return;
        }
        setAssigning(true);
        try {
    ...
    return (<div className="space-y-6">
      ...
      {activeSection === 'overview' && (<div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            ].map(action => (<button key={action.label} onClick={action.onClick} className="flex items-center gap-3 p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
      ...
      {/* Classes */}
      {activeSection === 'classes' && (<div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {!classes || classes.length === 0 ? (<tr><td colSpan={5} className="py-12 text-center text-gray-400">No classes found</td></tr>) : classes.map((cls) => (<tr key={cls.id} className="hover:bg-gray-50">
      ...
      {/* Departments */}
      {activeSection === 'departments' && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {(!departments || departments.length === 0) && (<div className="col-span-3 text-center py-12 text-gray-400">No departments configured</div>)}
          {departments?.map((dept) => (<div key={dept.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
      ...
      {/* Timetable */}
      {activeSection === 'timetable' && (<div className="space-y-4">
          {/* Class selector */}
          {classes && classes.length > 0 && (<div className="flex items-center gap-3">
              ...
              <select value={classId ?? ''} onChange={e => setSelectedClassId(e.target.value || null)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                {classes.map((cls) => (<option key={cls.id} value={cls.id}>{cls.name}{cls.grade ? ` — Grade ${cls.grade}` : ''}{cls.section ? ` (${cls.section})` : ''}</option>))}
              </select>
            </div>)}
                ...
                <table className="w-full text-sm min-w-[700px]">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      {DAYS.map(d => (<th key={d} className="py-3 px-4 font-medium text-gray-600 text-center">{d}</th>))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {TIME_SLOTS.map((time, i) => (<tr key={time} className="hover:bg-gray-50">
                        <td className="py-2 px-4 text-gray-500 font-medium text-xs whitespace-nowrap">{time}</td>
                        {DAYS.map(day => {
                        const slot = timetable.find(t => t.day?.toUpperCase() === day.toUpperCase() &&
                            (t.startTime === time || t.period === i + 1 || t.timeSlot === time));
                        return (<td key={day} className="py-1.5 px-2">
                              {slot ? (<div className="bg-blue-50 border border-blue-100 rounded-lg p-2 text-center">
                                  ...
                                  {slot.teacher && (<p className="text-xs text-blue-500 mt-0.5 truncate">
                                      {slot.teacher?.user?.firstName ?? slot.teacher?.name ?? slot.teacher}
                                    </p>)}
                                </div>) : (<div className="h-14 rounded-lg bg-gray-50 border border-dashed border-gray-200"/>)}
                            </td>);
                    })}
                      </tr>))}
                  </tbody>
      ...
      {/* Class Roster */}
      {activeSection === 'roster' && (<div className="space-y-4">
          {/* Class selector */}
          {classes && classes.length > 0 && (<div className="flex items-center gap-3">
              <label className="text-sm font-medium text-gray-700 shrink-0">Select Class:</label>
              <select value={classId ?? ''} onChange={e => setSelectedClassId(e.target.value || null)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
          ...
          {/* Assign student */}
          {classId && (<div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-sm font-medium text-gray-700 mb-3">Assign Student to Class</p>
              <div className="flex gap-3">
                <input type="text" placeholder="Enter Student ID..." value={assignStudentId} onChange={e => setAssignStudentId(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAssignStudent()} className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                <button onClick={handleAssignStudent} disabled={!assignStudentId.trim() || assigning} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none transition-colors">
