...
export default function AttendancePage() {
    ...
    const { data: schools } = useQuery({
        ...
        queryFn: () => api.get('/school-erp/schools').then((r) => r.data.data),
    });
    ...
    const markMutation = useMutation({
        mutationFn: (records) => api.post('/attendance', { classId, date, records }),
        onSuccess: () => {
            setSaved(true);
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        {TABS.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors', activeTab === tab.id
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {schools && schools.length > 1 && (<div className="flex items-center gap-2">
            ...
            <select value={schoolId ?? ''} onChange={(e) => {
              ...
              {schools.map((s) => (<option key={s.id} value={s.id}>
                  {s.name}
                </option>))}
            </select>
          </div>)}
        ...
        {activeTab === 'mark' && (<div className="flex items-center gap-2">
            <label className="text-sm font-medium text-gray-700 shrink-0">Date:</label>
            <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"/>
          </div>)}
        ...
        {activeTab === 'summary' && (<>
            <div className="flex items-center gap-2">
              ...
              <input type="date" value={summaryFrom} onChange={(e) => setSummaryFrom(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"/>
            </div>
            <div className="flex items-center gap-2">
              <label className="text-sm font-medium text-gray-700 shrink-0">To:</label>
              <input type="date" value={summaryTo} onChange={(e) => setSummaryTo(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"/>
            </div>
          </>)}
      </div>

      {/* Mark Attendance Tab */}
      {activeTab === 'mark' && (<div className="space-y-4">
          {!classId ? (<div className="bg-white rounded-xl border border-gray-200 py-16 text-center text-gray-400">
              No class selected
            </div>) : (<>
              {/* Bulk action buttons */}
              <div className="flex flex-wrap items-center gap-2">
                ...
                {Object.keys(STATUS_CONFIG).map((status) => {
                    ...
                    const Icon = cfg.icon;
                    return (<button key={status} onClick={() => setAllStatus(status)} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors', cfg.bg, cfg.color)}>
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                {rosterLoading ? (<div className="py-16 text-center text-gray-400">Loading students...</div>) : !roster || roster.length === 0 ? (<div className="py-16 text-center">
                        ...
                        return (<tr key={student.id} className="hover:bg-gray-50">
                            <td className="py-3 px-4">
                              <div className="flex items-center gap-3">
                                {avatarUrl ? (<img src={avatarUrl} alt={initials} className="h-8 w-8 rounded-full object-cover shrink-0"/>) : (<div className="h-8 w-8 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-xs font-semibold shrink-0">
                                    {initials}
                                  </div>)}
                                <div>
                                  ...
                                  {student.studentId && (<p className="text-xs text-gray-400 font-mono">
                                      {student.studentId}
                                    </p>)}
                                </div>
                              </div>
                            ...
                            <td className="py-3 px-4">
                              <div className="flex gap-1 flex-wrap">
                                ...
                                return (<button key={status} onClick={() => setAttendanceMap((prev) => ({
                                        ...prev,
                                        [student.id]: status,
                                    }))} title={cfg.label} className={cn('flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-colors', isActive
                            ...
                            <td className="py-3 px-4 hidden md:table-cell">
                              <input type="text" placeholder="Optional note..." value={noteMap[student.id] ?? ''} onChange={(e) => setNoteMap((prev) => ({
                                ...prev,
              ...
              {/* Save button */}
              {roster && roster.length > 0 && (<div className="flex items-center justify-between">
                  {saved ? (<span className="flex items-center gap-1.5 text-sm text-green-600 font-medium">
                      <Check className="h-4 w-4"/> Attendance saved successfully
                    </span>) : (<span />)}
                  <button onClick={handleSave} disabled={markMutation.isPending} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none transition-colors">
      ...
      {activeTab === 'summary' && (<div className="space-y-4">
          {!classId ? (<div className="bg-white rounded-xl border border-gray-200 py-16 text-center text-gray-400">
              No class selected
            </div>) : summaryLoading ? (<div className="bg-white rounded-xl border border-gray-200 py-16 text-center text-gray-400">
              Loading summary...
            </div>) : !summary || summary.length === 0 ? (<div className="bg-white rounded-xl border border-gray-200 py-16 text-center text-gray-400">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    ...
                    const initials = row.name
                        ...
                        .toUpperCase()
                        .slice(0, 2);
                    return (<tr key={row.studentId} className="hover:bg-gray-50">
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-3">
                              {row.avatarUrl ? (<img src={row.avatarUrl} alt={initials} className="h-8 w-8 rounded-full object-cover shrink-0"/>) : (<div className="h-8 w-8 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 flex items-center justify-center text-white text-xs font-semibold shrink-0">
                                  {initials}
                                </div>)}
                          ...
                          <td className="py-3 px-4 text-center">
                            <div className="flex items-center justify-center gap-2">
                              <div className="h-1.5 w-16 bg-gray-100 rounded-full overflow-hidden">
                                <div className={cn('h-full rounded-full', row.rate >= 90
                            ? 'bg-green-500'
                            : row.rate >= 75
                                ? 'bg-amber-500'
                                : 'bg-red-500')} style={{ width: `${row.rate}%` }}/>
                              </div>
                              <span className={cn('text-xs font-semibold', row.rate >= 90
                            ? 'text-green-700'
                            : row.rate >= 75
                                ? 'text-amber-700'
                                : 'text-red-700')}>
