...
function AddEntryModal({ classId, onClose, onSaved }: AddEntryModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: (dto: { dayOfWeek: number; startTime: string; endTime: string; room?: string; teacherId?: string }) =>
      api.post(`/school-erp/classes/${classId}/timetable`, dto).then((r) => r.data),
    onSuccess: () => {
      onSaved();
      onClose();
    },
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-50 disabled:pointer-events-none"
            >
  ...
  return (
    <div className="animate-pulse">
      <div className="grid grid-cols-6 gap-px bg-gray-200 rounded-xl overflow-hidden">
        ...
        {DAYS.map((d) => (
          <div key={d.value} className="bg-gray-50 p-3">
        ...
        {HOURS.slice(0, 8).map((h) => (
          <>
            ...
            {DAYS.map((d) => (
              <div key={`c-${h}-${d.value}`} className="bg-white p-1 min-h-[60px]" />
            ))}
          </>
        ))}
      </div>
...
export default function TimetablePage() {
  ...
  const { data: entries, isLoading } = useQuery<TimetableEntry[]>({
    ...
    queryFn: () =>
      api.get(`/school-erp/classes/${classId}/timetable`).then((r) => {
        const d = r.data;
        return Array.isArray(d) ? d : d?.data ?? [];
      }),
    enabled: !!classId,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/school-erp/timetable/${id}`),
    onSuccess: () => {
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse min-w-[700px]">
                  ...
                  {DAYS.map((d) => {
                    ...
                    return (
                      <th
                        key={d.value}
                        className={cn(
                          'py-3 px-2 text-center text-xs font-semibold uppercase tracking-wide',
                          colors.text,
                        )}
                      >
                        {d.label}
              ...
              <tbody>
                {HOURS.map((hour) => (
                  <tr key={hour} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/50">
                    {/* Time label */}
                    ...
                    {DAYS.map((d) => {
                      ...
                      return (
                        <td
                          key={d.value}
                          className="py-1.5 px-1.5 align-top min-w-[120px] min-h-[56px]"
                        >
                          <div className="flex flex-col gap-1">
                            {cellEntries.map((entry) => (
                              <div
                                key={entry.id}
                                className={cn(
                                  'relative group rounded-lg border px-2 py-1.5 text-xs leading-snug',
                                  colors.bg,
                                  colors.text,
                                  colors.border,
                                )}
                              >
                                ...
                                {entry.room && entry.startTime && (
                                  <p className="opacity-60 text-[10px]">Room: {entry.room}</p>
                                )}
                              </div>
                            ))}
                          </div>
