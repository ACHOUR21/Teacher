...
export default function MyLearningPage() {
  ...
  const inProgress = (progress ?? []).filter((p: any) => !p.completedAt);
  const completed = (progress ?? []).filter((p: any) => !!p.completedAt);
        ...
        {progressLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {inProgress.map((p: any) => (
              <CourseProgressCard key={p.id} progress={p} />
            ))}
          </div>
        )}
      </div>
...
function CourseProgressCard({ progress }: { progress: any }) {
  ...
  return (
    <Link
      ...
      className="block bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md hover:border-blue-200 transition-all"
    >
      {/* Thumbnail */}
