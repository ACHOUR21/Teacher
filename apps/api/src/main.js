...
var _platformSocket = require("@nestjs/platform-socket.io");
var _swagger = require("@nestjs/swagger");
var _compression = _interopRequireDefault(require("compression"));
var _helmet = _interopRequireDefault(require("helmet"));
var _app = require("./app.module");
var _allExceptions = require("./modules/core/filters/all-exceptions.filter");
var _logging = require("./modules/core/interceptors/logging.interceptor");
var _timeout = require("./modules/core/interceptors/timeout.interceptor");
var _transform = require("./modules/core/interceptors/transform.interceptor");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
// Sentry and OpenTelemetry must be initialized before any other imports
// eslint-disable-next-line import/order

(0, _instrumentation.startSentry)();
(0, _instrumentation.startTelemetry)();
async function bootstrap() {
  ...
  app.useGlobalFilters(new _allExceptions.AllExceptionsFilter());
  app.useGlobalInterceptors(new _logging.LoggingInterceptor(), new _transform.TransformInterceptor(), new _timeout.TimeoutInterceptor());
  ...
  // Socket.IO
  app.useWebSocketAdapter(new _platformSocket.IoAdapter(app));
  // Swagger
...
This document describes **v1** of the EduAI API. All routes are prefixed with \`/api/v1\`.
  `.trim();
  const config = new _swagger.DocumentBuilder().setTitle('EduAI Ultimate API').setDescription(swaggerDescription).setVersion(apiVersion).setContact('EduAI Support', 'https://eduai.app', 'support@eduai.app').setLicense('Proprietary', 'https://eduai.app/terms').setExternalDoc('Platform Documentation', 'https://docs.eduai.app').addServer('http://localhost:3001', 'Local Development').addServer('https://api.eduai.app', 'Production').addBearerAuth({
    type: 'http',
    scheme: 'bearer',
    bearerFormat: 'JWT',
    name: 'JWT',
  ...
  // Core platform
  .addTag('Health', 'Liveness and readiness health checks').addTag('Auth', 'Authentication — login, register, refresh, logout, MFA').addTag('Tenants', 'Multi-tenant provisioning, settings, and plan management').addTag('Users', 'User CRUD, profile, roles, device management')
  // Education
  .addTag('Courses', 'Course authoring — sections, lessons, publishing, pricing').addTag('Students', 'Student profiles, progress, performance, submissions').addTag('Teachers', 'Teacher profiles, verification, subject management').addTag('Parents', 'Parent portal — child linking, monitoring, communications')
  // ERP
  .addTag('School ERP', 'Schools, departments, classes, timetables, attendance').addTag('University ERP', 'Universities, faculties, programs, academic enrollments')
  // AI
  .addTag('AI', 'AI Education Suite — tutor, exam/lesson/curriculum generation, flashcards, mind maps').addTag('AI Agents', 'AI Agents Platform — autonomous agents, tool orchestration, pipelines')
  // Commerce & interaction
  .addTag('Marketplace', 'Course discovery, purchase flow, ratings and reviews').addTag('Live', 'Live classroom sessions, recordings, participant management').addTag('Certificates', 'Certificate template design, issuance, public verification').addTag('Gamification', 'Points, XP, badges, leaderboards, streaks, events')
  // Communication
  .addTag('Notifications', 'In-app, email, SMS and push notification delivery').addTag('Messaging', 'Real-time direct and group messaging via WebSocket')
  // Platform services
  .addTag('Billing', 'Subscriptions, plans, Stripe/PayPal, coupons, invoices, revenue analytics').addTag('Analytics', 'Platform analytics, BI dashboards, reports and data export').addTag('White Label', 'Custom branding, domain mapping, email themes').addTag('Plugins', 'Plugin marketplace — discovery, installation, configuration').addTag('API Ecosystem', 'API key management, webhook endpoints, rate limiting').addTag('Search', 'Full-text search across courses, users, and content (Elasticsearch)').addTag('Storage', 'File upload, presigned URLs, S3-compatible object storage').addTag('Audit', 'Immutable audit log — actions, resources, actors, timestamps').build();
