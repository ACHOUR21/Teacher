...
describe('AiAgentsService', () => {
  ...
  beforeEach(async () => {
    ...
    Anthropic.mockImplementation(() => ({
      messages: {
        create: anthropicMock
      }
    }));
    const module = await _testing.Test.createTestingModule({
      providers: [_aiAgents.AiAgentsService, {
    ...
    service = module.get(_aiAgents.AiAgentsService);
    jest.clearAllMocks();
  });
  ...
  describe('chat', () => {
    it('should create a new conversation and return a text response', async () => {
      ...
      mockPrisma.aIUsage.create.mockResolvedValueOnce({
        id: 'usage-1'
      });
      anthropicMock.mockResolvedValueOnce({
        content: [{
          type: 'text',
          text: 'Here is a hint for your problem.'
        }],
        usage: {
          input_tokens: 100,
          output_tokens: 50
