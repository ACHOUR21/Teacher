...
function makePagination(page = 1, limit = 20, extra = {}) {
  const dto = new _pagination.PaginationDto();
  dto.page = page;
  dto.limit = limit;
...
describe('CoursesService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_courses.CoursesService, {
        ...
        provide: _email.EmailService,
        useValue: mockEmailService
      }]
    }).compile();
    service = module.get(_courses.CoursesService);
    jest.clearAllMocks();
  });
  // ─── findAll ──────────────────────────────────────────────────────────────
  describe('findAll', () => {
    ...
    it('should filter by level when provided', async () => {
      ...
      await service.findAll('tenant-1', makePagination(1, 10), {
        level: 'BEGINNER'
      });
      expect(mockPrisma.course.findMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException when course does not exist', async () => {
      ...
      await expect(service.findById('nonexistent', 'tenant-1')).rejects.toThrow(_common.NotFoundException);
    });
  });
  // ─── create ───────────────────────────────────────────────────────────────
  describe('create', () => {
    it('should create a new course', async () => {
      ...
      expect(mockPrisma.course.create).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('update', () => {
    it('should throw NotFoundException when course not found', async () => {
      mockPrisma.course.findFirst.mockResolvedValueOnce(null);
      await expect(service.update('nonexistent', 'tenant-1', {
        title: 'Updated'
    ...
    it('should throw ForbiddenException when teacher tries to edit another teacher course', async () => {
      ...
      mockPrisma.teacher.findFirst.mockResolvedValueOnce({
        id: 'teacher-1',
        userId: 'user-1'
      });
      ...
      await expect(service.update('course-1', 'tenant-1', {
        title: 'Hacked'
      }, 'user-1')).rejects.toThrow(_common.ForbiddenException);
    });
    it('should allow admin to edit any course without ForbiddenException', async () => {
      ...
      const result = await service.update('course-1', 'tenant-1', {
        title: 'Admin Edit'
      }, 'user-admin');
  ...
  describe('publish', () => {
    it('should throw NotFoundException when course not found', async () => {
      mockPrisma.course.findFirst.mockResolvedValueOnce(null);
      await expect(service.publish('nonexistent', 'tenant-1')).rejects.toThrow(_common.NotFoundException);
    });
  ...
  describe('unpublish', () => {
    it('should throw NotFoundException when course not found', async () => {
      mockPrisma.course.findFirst.mockResolvedValueOnce(null);
      await expect(service.unpublish('nonexistent', 'tenant-1')).rejects.toThrow(_common.NotFoundException);
    });
  ...
  describe('enrollStudent', () => {
    it('should throw NotFoundException when course is not published', async () => {
      mockPrisma.course.findFirst.mockResolvedValueOnce(null);
      await expect(service.enrollStudent('course-1', 'student-1', 'tenant-1')).rejects.toThrow(_common.NotFoundException);
    });
    ...
    it('should create progress record and increment enrollCount on success', async () => {
      ...
      expect(mockPrisma.courseProgress.create).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({
          studentId: 'student-1',
          courseId: 'course-1'
        })
  ...
  describe('createSection', () => {
    it('should throw NotFoundException when course not found', async () => {
      mockPrisma.course.findFirst.mockResolvedValueOnce(null);
      await expect(service.createSection('nonexistent', 'tenant-1', {
        title: 'Intro',
        position: 1
    ...
    it('should create section with correct courseId and invalidate cache', async () => {
      ...
      const result = await service.createSection('course-1', 'tenant-1', {
        title: 'Intro',
        position: 1
      });
      expect(result.id).toBe('section-1');
      expect(mockPrisma.courseSection.create).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({
          courseId: 'course-1',
          title: 'Intro'
        })
  ...
  describe('createLesson', () => {
    it('should throw NotFoundException when section not found', async () => {
      ...
      await expect(service.createLesson('nonexistent-section', 'tenant-1', {
        title: 'Lesson 1',
        contentType: 'VIDEO',
        position: 1
      })).rejects.toThrow(_common.NotFoundException);
    ...
    it('should create lesson and update totalLessons count', async () => {
      ...
      expect(mockPrisma.lesson.count).toHaveBeenCalledWith(expect.objectContaining({
        where: {
          section: {
            courseId: 'course-1'
          }
  ...
  describe('addReview', () => {
    it('should throw ConflictException when user already reviewed this course', async () => {
      ...
      await expect(service.addReview('course-1', 'user-1', 4, 'Great!')).rejects.toThrow(_common.ConflictException);
    });
    it('should throw BadRequestException for rating below 1', async () => {
      mockPrisma.review.findUnique.mockResolvedValueOnce(null);
      await expect(service.addReview('course-1', 'user-1', 0)).rejects.toThrow(new _common.BadRequestException('Rating must be between 1 and 5'));
    });
    it('should throw BadRequestException for rating above 5', async () => {
      mockPrisma.review.findUnique.mockResolvedValueOnce(null);
      await expect(service.addReview('course-1', 'user-1', 6)).rejects.toThrow(new _common.BadRequestException('Rating must be between 1 and 5'));
    });
