...
describe('DashboardService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_dashboard.DashboardService, {
    ...
    service = module.get(_dashboard.DashboardService);
    jest.clearAllMocks();
  });
  ...
  describe('getTenantOverview', () => {
    ...
    it('should query DB and cache result on cache miss', async () => {
      ...
      mockPrisma.user.count.mockResolvedValueOnce(150) // totalUsers
      ...
      expect(mockCache.set).toHaveBeenCalledWith(`dashboard:overview:${tenantId}`, expect.any(String), 60);
    });
  ...
  describe('getTopContent', () => {
    it('should return top 5 courses by enrollment and by rating', async () => {
      ...
      mockPrisma.course.findMany.mockResolvedValueOnce(byEnrollment).mockResolvedValueOnce(byRating);
  ...
  describe('getUserMetrics', () => {
    it('should return dau, wau, mau and daily breakdown', async () => {
      mockPrisma.user.count.mockResolvedValueOnce(10) // dau
    ...
    it('should clamp days between 1 and 365', async () => {
      mockPrisma.user.count.mockResolvedValueOnce(0).mockResolvedValueOnce(0).mockResolvedValueOnce(0);
      mockPrisma.user.findMany.mockResolvedValueOnce([]);
  ...
  describe('getRevenueMetrics', () => {
    it('should compute MRR and ARR for an active subscription', async () => {
      ...
      mockPrisma.subscription.count.mockResolvedValueOnce(1) // churnCount
    ...
    it('should group invoices into daily revenue buckets', async () => {
      ...
      mockPrisma.invoice.findMany.mockResolvedValueOnce([{
        amount: '29',
        paidAt: new Date(`${yesterdayStr}T00:00:00Z`)
      }, {
        amount: '29',
        paidAt: new Date(`${yesterdayStr}T12:00:00Z`)
      }]);
      mockPrisma.subscription.count.mockResolvedValueOnce(0).mockResolvedValueOnce(0);
      const result = await service.getRevenueMetrics('tenant-1', 30);
