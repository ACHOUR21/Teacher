...
describe('PluginsService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_plugins.PluginsService, {
        provide: _prisma.PrismaService,
        useValue: mockPrisma
      }, {
        provide: _pluginSandbox.PluginSandboxService,
        useValue: {
    ...
    service = module.get(_plugins.PluginsService);
    jest.clearAllMocks();
  });
  describe('listMarketplace', () => {
    it('should return paginated plugins ordered by installCount', async () => {
      ...
      const result = await service.listMarketplace('tenant-1', {
        page: 1,
        limit: 20
      });
    ...
    it('should apply search filter', async () => {
      ...
      await service.listMarketplace('tenant-1', {
        search: 'quiz'
      });
      expect(mockPrisma.plugin.findMany).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('installPlugin', () => {
    it('should install a plugin and increment installCount', async () => {
      mockPrisma.plugin.findUnique.mockResolvedValueOnce({
        id: 'p-1',
        name: 'Quiz Builder'
      });
    ...
    it('should throw NotFoundException for unknown plugin', async () => {
      ...
      await expect(service.installPlugin('tenant-1', 'bad-plugin')).rejects.toThrow(_common.NotFoundException);
    });
    it('should throw ConflictException when plugin already installed', async () => {
      mockPrisma.plugin.findUnique.mockResolvedValueOnce({
        id: 'p-1'
      });
      mockPrisma.installedPlugin.findUnique.mockResolvedValueOnce({
        id: 'ip-1'
      }); // already installed
      await expect(service.installPlugin('tenant-1', 'p-1')).rejects.toThrow(_common.ConflictException);
    });
  });
  describe('uninstallPlugin', () => {
    ...
    it('should throw NotFoundException when plugin not installed', async () => {
      ...
      await expect(service.uninstallPlugin('tenant-1', 'p-1')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('togglePlugin', () => {
    ...
    it('should throw NotFoundException when plugin not installed', async () => {
      mockPrisma.installedPlugin.findUnique.mockResolvedValueOnce(null);
      await expect(service.togglePlugin('tenant-1', 'p-1', true)).rejects.toThrow(_common.NotFoundException);
    });
  });
