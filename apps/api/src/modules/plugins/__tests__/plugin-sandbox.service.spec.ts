...
describe('PluginSandboxService', () => {
  ...
  describe('validateManifest', () => {
    ...
    it('throws when manifest is null', () => {
      expect(() => service.validateManifest(null)).toThrow(BadRequestException);
    });

    it('throws when manifest is a string', () => {
      expect(() => service.validateManifest('bad')).toThrow(BadRequestException);
    });
    ...
    it('throws when version is empty string', () => {
      expect(() => service.validateManifest({ ...validManifest, version: '   ' })).toThrow('version');
    });

    it('throws when entrypoint is missing', () => {
      const { entrypoint: _, ...rest } = validManifest;
      expect(() => service.validateManifest(rest)).toThrow('entrypoint');
    });

    it('throws when entrypoint is an external http URL', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, entrypoint: 'http://evil.com/xss.js' }),
      ).toThrow('External entrypoint');
    });

    it('throws when entrypoint contains path traversal', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, entrypoint: '../../etc/passwd' }),
      ).toThrow('".."');
    });

    it('throws when permissions is not an array', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, permissions: 'read:courses' }),
      ).toThrow('permissions array');
    });

    it('throws when permissions contains unknown permission', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, permissions: ['read:courses', 'destroy:everything'] }),
      ).toThrow('Unknown permissions');
    });

    it('accepts an empty permissions array', () => {
      expect(() => service.validateManifest({ ...validManifest, permissions: [] })).not.toThrow();
    });

    it('throws when sandboxFlags is not an array', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, sandboxFlags: 'allow-scripts' }),
      ).toThrow('sandboxFlags');
    });

    it('throws for dangerous flag allow-same-origin', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, sandboxFlags: ['allow-same-origin'] }),
      ).toThrow('allow-same-origin');
    });

    it('throws for dangerous flag allow-top-navigation', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, sandboxFlags: ['allow-top-navigation'] }),
      ).toThrow('allow-top-navigation');
    });

    it('throws for unknown sandbox flag', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, sandboxFlags: ['allow-pointer-lock'] }),
      ).toThrow('Unrecognised sandbox flags');
    });

    it('accepts valid optional sandbox flags', () => {
      expect(() =>
        service.validateManifest({ ...validManifest, sandboxFlags: ['allow-forms', 'allow-popups'] }),
      ).not.toThrow();
    });
  });
  ...
  describe('checkRequiredPermissions', () => {
    it('passes when all required permissions are declared', () => {
      expect(() =>
        service.checkRequiredPermissions(validManifest, ['read:courses']),
      ).not.toThrow();
    });

    it('throws when a required permission is missing', () => {
      expect(() =>
        service.checkRequiredPermissions(validManifest, ['write:grades']),
      ).toThrow('write:grades');
    });

    it('passes with empty required array', () => {
      expect(() =>
        service.checkRequiredPermissions(validManifest, []),
      ).not.toThrow();
    });
  });
  ...
  describe('buildSandboxAttribute', () => {
    ...
    it('appends extra flags from manifest', () => {
      const manifest = { ...validManifest, sandboxFlags: ['allow-forms'] as any };
      expect(service.buildSandboxAttribute(manifest)).toContain('allow-forms');
    });

    it('deduplicates repeated flags', () => {
      const manifest = { ...validManifest, sandboxFlags: ['allow-scripts'] as any };
  ...
  describe('buildCsp', () => {
    ...
    it('adds blob: to connect-src when access:storage is granted', () => {
      const manifest = { ...validManifest, permissions: ['access:storage'] as any };
