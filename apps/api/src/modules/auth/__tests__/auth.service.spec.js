...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get(_auth.AuthService);
    jest.clearAllMocks();
  ...
  describe('createTenantAndAdmin', () => {
    it('should throw ConflictException when tenant slug already exists', async () => {
      ...
      await expect(service.createTenantAndAdmin({
    ...
    it('should create tenant and admin user on success', async () => {
      ...
      mockPrisma.$transaction.mockImplementationOnce(async cb => {
        ...
        mockPrisma.userProfile.create.mockResolvedValueOnce({
          id: 'profile-1'
        });
  ...
  describe('register', () => {
    ...
    it('should throw ConflictException when user already exists', async () => {
      ...
      await expect(service.register(registerCmd)).rejects.toThrow(_common.ConflictException);
    });
    ...
    it('should call sendWelcomeEmail as fire-and-forget on success', async () => {
      ...
      expect(mockNotifications.sendWelcomeEmail).toHaveBeenCalledWith('newuser@test.com', 'New', 'Test School');
    });
  });
  // ---------------------------------------------------------------------------
  // login
  ...
  describe('login', () => {
    ...
    it('should return requiresMfa:true and MFA-Challenge token when mfaEnabled is true', async () => {
      ...
      expect(mockRedis.set).toHaveBeenCalledWith(expect.stringMatching(/^mfa:challenge:/), 'user-1', 300);
    });
  });
  // ---------------------------------------------------------------------------
  // refreshTokens
  ...
  describe('refreshTokens', () => {
    it('should throw UnauthorizedException when session not found', async () => {
      ...
      await expect(service.refreshTokens('invalid-refresh-token')).rejects.toThrow(_common.UnauthorizedException);
    });
    it('should throw UnauthorizedException when session is expired', async () => {
      ...
      await expect(service.refreshTokens('old-refresh-token')).rejects.toThrow(_common.UnauthorizedException);
    });
    it('should rotate refresh token: delete old session, create new one, return new tokens', async () => {
      ...
      mockPrisma.userSession.delete.mockResolvedValueOnce({
        id: 'session-1'
      });
  ...
  describe('logout', () => {
    it('should delete specific session when refreshToken provided', async () => {
      mockPrisma.userSession.deleteMany.mockResolvedValueOnce({
        count: 1
      });
  ...
  describe('changePassword', () => {
    it('should throw UnauthorizedException when current password is wrong', async () => {
      ...
      await expect(service.changePassword('user-1', 'wrong-password', 'NewPassword123!')).rejects.toThrow(_common.UnauthorizedException);
    });
  ...
  describe('forgotPassword', () => {
    it('should silently return without throwing when user not found', async () => {
      mockPrisma.user.findUnique.mockResolvedValueOnce(null);
      await expect(service.forgotPassword('unknown@test.com', 'tenant-1')).resolves.toBeUndefined();
    ...
    it('should store reset token in redis and call sendPasswordResetEmail when user found', async () => {
      ...
      expect(mockRedis.set).toHaveBeenCalledWith(expect.stringMatching(/^pwd:reset:/), 'user-1', 3600);
      await Promise.resolve(); // flush fire-and-forget
      expect(mockNotifications.sendPasswordResetEmail).toHaveBeenCalledWith('user@test.com', 'Test', expect.any(String));
    });
  });
  // ---------------------------------------------------------------------------
  // verifyEmail
  ...
  describe('verifyEmail', () => {
    it('should throw BadRequestException when token not found in redis', async () => {
      ...
      await expect(service.verifyEmail('invalid-token')).rejects.toThrow(_common.BadRequestException);
      await expect(service.verifyEmail('invalid-token')).rejects.toThrow('Invalid or expired verification token');
    });
  ...
  describe('resetPassword', () => {
    it('should throw BadRequestException when token not found in redis', async () => {
      mockRedis.get.mockResolvedValueOnce(null);
      await expect(service.resetPassword('bad-token', 'NewPassword!')).rejects.toThrow(_common.BadRequestException);
      await expect(service.resetPassword('bad-token', 'NewPassword!')).rejects.toThrow('Invalid or expired reset token');
    });
