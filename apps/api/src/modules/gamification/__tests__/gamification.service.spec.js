...
describe('GamificationService', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await _testing.Test.createTestingModule({
      providers: [_gamification.GamificationService, {
    ...
    service = module.get(_gamification.GamificationService);
  });
  describe('awardPoints', () => {
    it('should upsert user points and return updated total', async () => {
      ...
      expect(mockPrisma.userPoints.upsert).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('processEvent', () => {
    ...
    it('triggers level-up notification when level changes', async () => {
      ...
      expect(mockNotifications.notifyUser).toHaveBeenCalledWith('u1', expect.stringContaining('Level up'), expect.any(String), expect.any(Object));
    });
  });
  ...
  describe('getMyStats', () => {
    it('returns weekly XP and streak from database', async () => {
      ...
      mockPrisma.gamificationEvent.aggregate.mockResolvedValue({
        _sum: {
          xpAwarded: 75
        }
      });
