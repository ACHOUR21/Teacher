...
var _resilience = require("../services/resilience.service");
describe('ResilienceService', () => {
  let service;
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_resilience.ResilienceService]
    }).compile();
    service = module.get(_resilience.ResilienceService);
  });
  // ─── withRetry ────────────────────────────────────────────────────────────
  describe('withRetry', () => {
    ...
    it('exhausts all retries and re-throws the last error', async () => {
      ...
      await expect(service.withRetry('test', fn, {
        maxAttempts: 3,
        baseDelayMs: 0
  ...
  describe('withCircuitBreaker', () => {
    ...
    it('throws ServiceUnavailableException when OPEN and no fallback is provided', async () => {
      ...
      await expect(service.withCircuitBreaker('svc', fn)).rejects.toBeInstanceOf(_common.ServiceUnavailableException);
    });
    it('transitions to HALF_OPEN after recovery timeout and closes on probe success', async () => {
      ...
      const circuit = service.circuits.get('svc');
  ...
  describe('withResilience', () => {
    it('combines retry and circuit breaker, returning result on eventual success', async () => {
      const fn = jest.fn().mockRejectedValueOnce(new Error('transient')).mockResolvedValue('success');
    ...
    it('invokes fallback when all retries are exhausted and circuit remains closed', async () => {
      ...
      const result = await service.withResilience('svc', fn, () => 'safe-fallback', {
        maxAttempts: 2,
        baseDelayMs: 0
      });
