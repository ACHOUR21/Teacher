...
describe('ParentsService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_parents.ParentsService, {
        provide: _prisma.PrismaService,
        useValue: mockPrisma
      }]
    }).compile();
    service = module.get(_parents.ParentsService);
    jest.clearAllMocks();
  });
  describe('getChildren', () => {
    ...
    it('should throw NotFoundException when parent profile not found', async () => {
      ...
      await expect(service.getChildren('bad-user')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('getChildProgress', () => {
    it('should return progress for a linked child', async () => {
      mockPrisma.parent.findUnique.mockResolvedValueOnce({
        id: 'p-1'
      });
      mockPrisma.parentStudent.findUnique.mockResolvedValueOnce({
        parentId: 'p-1',
        studentId: 's-1'
      });
    ...
    it('should throw ForbiddenException when not linked to student', async () => {
      ...
      await expect(service.getChildProgress('parent-user-1', 'unrelated-student')).rejects.toThrow(_common.ForbiddenException);
    });
  });
  describe('linkChild', () => {
    ...
    it('should throw NotFoundException when parent profile does not exist', async () => {
      mockPrisma.parent.findUnique.mockResolvedValueOnce(null);
      await expect(service.linkChild('bad-parent', 's-1')).rejects.toThrow(_common.NotFoundException);
    });
    it('should throw NotFoundException when student does not exist', async () => {
      ...
      await expect(service.linkChild('parent-user-1', 'bad-student')).rejects.toThrow(_common.NotFoundException);
    });
  });
});