...
describe('QuizzesService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_quizzes.QuizzesService, {
    ...
    service = module.get(_quizzes.QuizzesService);
    jest.clearAllMocks();
  });
  ...
  describe('getById', () => {
    ...
    it('should throw NotFoundException when quiz does not exist', async () => {
      ...
      await expect(service.getById('nonexistent')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('create', () => {
  ...
  describe('update', () => {
    it('should update quiz title and questions', async () => {
      ...
      const result = await service.update('quiz-1', {
        title: 'New Title'
      });
  ...
  describe('deleteQuiz', () => {
    it('should delete an existing quiz', async () => {
      ...
      mockPrisma.quiz.delete.mockResolvedValueOnce({
        id: 'quiz-1'
      });
    ...
    it('should throw NotFoundException when quiz does not exist', async () => {
      mockPrisma.quiz.findUnique.mockResolvedValueOnce(null);
      await expect(service.deleteQuiz('nonexistent')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('submitAttempt', () => {
    ...
    it('should grade all correct answers and return 100% score', async () => {
      mockPrisma.quiz.findUnique.mockResolvedValueOnce(mockQuiz);
      mockPrisma.quizAttempt.create.mockResolvedValueOnce({
        id: 'attempt-1',
        score: 100,
        passed: true
      });
    ...
    it('should notify user on passing score', async () => {
      ...
      expect(mockNotifications.notifyUser).toHaveBeenCalledWith('user-1', 'Quiz Passed!', expect.stringContaining('Test Quiz'), expect.any(Object));
    });
    ...
    it('should throw NotFoundException when quiz does not exist', async () => {
      mockPrisma.quiz.findUnique.mockResolvedValueOnce(null);
      await expect(service.submitAttempt('nonexistent', 'user-1', [])).rejects.toThrow(_common.NotFoundException);
    });
    it('should throw BadRequestException when quiz has no questions', async () => {
      ...
      await expect(service.submitAttempt('quiz-empty', 'user-1', [])).rejects.toThrow(_common.BadRequestException);
    });
  ...
  describe('getMyAttempts', () => {
    ...
    it('should throw NotFoundException when quiz does not exist', async () => {
      mockPrisma.quiz.findUnique.mockResolvedValueOnce(null);
      await expect(service.getMyAttempts('nonexistent', 'user-1')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('getResults', () => {
    ...
    it('should throw NotFoundException when quiz does not exist', async () => {
      mockPrisma.quiz.findUnique.mockResolvedValueOnce(null);
      await expect(service.getResults('nonexistent')).rejects.toThrow(_common.NotFoundException);
    });
  });
});