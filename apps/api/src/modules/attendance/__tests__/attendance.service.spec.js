...
describe('AttendanceService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_attendance.AttendanceService, {
        provide: _prisma.PrismaService,
        useValue: mockPrisma
      }]
    }).compile();
    service = module.get(_attendance.AttendanceService);
    jest.clearAllMocks();
  });
  // ---------------------------------------------------------------------------
  // markAttendance
  ...
  describe('markAttendance', () => {
    it('happy path: calls $transaction with one upsert promise per record', async () => {
      const upsertResult1 = {
        id: 'a1',
        studentId: 's1',
        status: _client.AttendanceStatus.PRESENT
      };
      const upsertResult2 = {
        id: 'a2',
        studentId: 's2',
      ...
      mockPrisma.attendance.upsert.mockReturnValueOnce(upsertResult1).mockReturnValueOnce(upsertResult2);
  ...
  describe('getStudentAttendance', () => {
    ...
    it('throws NotFoundException when student is not found', async () => {
      ...
      await expect(service.getStudentAttendance('unknown-id')).rejects.toThrow(new _common.NotFoundException('Student not found'));
  ...
  describe('getClassSummary', () => {
    it('throws NotFoundException when class is not found', async () => {
      ...
      await expect(service.getClassSummary('missing-class', '2024-01-01', '2024-01-31')).rejects.toThrow(new _common.NotFoundException('Class not found'));
      expect(mockPrisma.attendance.findMany).not.toHaveBeenCalled();
    });
    it('correct rate: 3 PRESENT + 1 LATE + 1 ABSENT = total 5, rate = 80%', async () => {
      ...
      mockPrisma.attendance.findMany.mockResolvedValueOnce([makeRecord(_client.AttendanceStatus.PRESENT), makeRecord(_client.AttendanceStatus.PRESENT), makeRecord(_client.AttendanceStatus.PRESENT), makeRecord(_client.AttendanceStatus.LATE), makeRecord(_client.AttendanceStatus.ABSENT)]);
    ...
    it('correctly counts each status type across multiple records', async () => {
      ...
      mockPrisma.attendance.findMany.mockResolvedValueOnce([makeRecord(_client.AttendanceStatus.PRESENT), makeRecord(_client.AttendanceStatus.ABSENT), makeRecord(_client.AttendanceStatus.LATE), makeRecord(_client.AttendanceStatus.EXCUSED), makeRecord(_client.AttendanceStatus.EXCUSED)]);
      ...
      mockPrisma.attendance.findMany.mockResolvedValueOnce([{
        ...
        student: {
          user: {
            firstName: 'Carol',
            lastName: 'White',
            avatarUrl: 'http://img.test/carol.png'
          }
        }
      }]);
      const result = await service.getClassSummary('class1', '2024-01-01', '2024-01-31');
  ...
  describe('getClassRoster', () => {
    it('throws NotFoundException when class is not found', async () => {
      mockPrisma.schoolClass.findUnique.mockResolvedValueOnce(null);
      await expect(service.getClassRoster('missing-class')).rejects.toThrow(new _common.NotFoundException('Class not found'));
    ...
    it('verifies schoolClass.findUnique is called with correct classId', async () => {
      ...
      expect(mockPrisma.schoolClass.findUnique).toHaveBeenCalledWith({
        where: {
          id: 'class-xyz'
        }
      });
