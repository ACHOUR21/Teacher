...
let AnalyticsService = exports.AnalyticsService = class AnalyticsService {
  ...
  async getPlatformStats(tenantId) {
    ...
    const [totalUsers, totalCourses, activeSessions, totalRevenue] = await Promise.all([this.prisma.user.count({
      where: {
        tenantId,
        isActive: true
      }
  ...
  async getUserGrowth(tenantId, days = 30) {
    const from = new Date();
  ...
  async getStudentActivity(tenantId, days = 7) {
    const from = new Date();
    from.setDate(from.getDate() - days);
    const [activeStudents, completions, submissions] = await Promise.all([this.prisma.courseProgress.count({
      where: {
        course: {
          tenantId
        },
  ...
  async getRevenueAnalytics(tenantId, months = 6) {
    const from = new Date();
  ...
  async getEngagementHeatmap(tenantId) {
    const from = new Date();
  ...
  async getTopCourses(tenantId) {
    ...
    const completionData = await Promise.all(courses.map(c => this.prisma.courseProgress.count({
      where: {
        courseId: c.id,
        completedAt: {
          not: null
  ...
  async getTeacherOverview(teacherId) {
    ...
    const completionCounts = await Promise.all(courses.map(c => this.prisma.courseProgress.count({
      where: {
        courseId: c.id,
        completedAt: {
          not: null
    ...
    const avgCompletionRate = courses.length > 0 ? courses.reduce((s, c, i) => s + (c._count.progress > 0 ? completionCounts[i] / c._count.progress * 100 : 0), 0) / courses.length : 0;
  ...
  async getTeacherCoursePerformance(teacherId) {
    const courses = await this.prisma.course.findMany({
      where: {
        teacherId
      },
    ...
    return courses.map((c, i) => ({
      ...
      // placeholder until quiz scores aggregated
      revenue: Math.round(Number(c.price) * c.enrollCount),
      rating: Number(c.rating).toFixed(1)
    }));
  }
  async getTeacherEnrollmentTrend(teacherId, days = 30) {
    const from = new Date();
    from.setDate(from.getDate() - days);
    const courses = await this.prisma.course.findMany({
      where: {
  ...
  async getTeacherTopStudents(teacherId) {
    const courses = await this.prisma.course.findMany({
      where: {
        teacherId
      },
  ...
  async getStudentOverview(studentId) {
    const [enrolledCourses, completedCourses, recentActivity] = await Promise.all([this.prisma.courseProgress.count({
      where: {
        studentId
      }
    }), this.prisma.courseProgress.count({
    ...
    const avgProgress = recentActivity.length > 0 ? recentActivity.reduce((s, a) => s + a.progressPercent, 0) / recentActivity.length : 0;
    return {
      enrolledCourses,
      completedCourses,
  ...
  async getStudentActivityHeatmap(studentId) {
    const from = new Date();
  ...
  async getAdminPlatformOverview(tenantId) {
    const from30 = new Date();
    from30.setDate(from30.getDate() - 30);
    const from60 = new Date();
    from60.setDate(from60.getDate() - 60);
    const [totalTenants, totalUsers, mauUsers, recentRevenue, prevRevenue, aiCostToday] = await Promise.all([this.prisma.tenant.count({
      where: {
        isActive: true
      }
  ...
  async getCohortRetention(tenantId) {
    const weeks = [1, 2, 3, 4];
    const cohorts = await Promise.all(weeks.map(async w => {
      ...
      const [day1Active, day7Active, day30Active] = await Promise.all([this.prisma.user.count({
        where: {
          id: {
            in: userIds
          },
        ...
        where: {
          ...
          lastLoginAt: {
            gte: new Date(start.getTime() + 6 * 86400000)
          }
        }
      }), this.prisma.user.count({
        where: {
          ...
          lastLoginAt: {
            gte: new Date(start.getTime() + 29 * 86400000)
          }
        }
      })]);
      return {
  ...
  async getTopTenants() {
    ...
    const revenueData = await Promise.all(tenants.map(t => this.prisma.invoice.aggregate({
      where: {
        subscription: {
          tenantId: t.id
        },
...
exports.AnalyticsService = AnalyticsService = __decorate([(0, _common.Injectable)(), __param(0, (0, _common.Inject)(_prisma.PrismaService)), __param(1, (0, _common.Inject)(_redis.RedisService)), __metadata("design:paramtypes", [Object, Object])], AnalyticsService);