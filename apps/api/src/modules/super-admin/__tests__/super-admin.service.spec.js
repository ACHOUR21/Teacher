...
describe('SuperAdminService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_superAdmin.SuperAdminService, {
    ...
    service = module.get(_superAdmin.SuperAdminService);
    jest.clearAllMocks();
  });
  ...
  function setupPlatformOverviewMocks(overrides = {}) {
    ...
    mockPrisma.tenant.count.mockResolvedValueOnce(totalTenants).mockResolvedValueOnce(activeTenants);
    mockPrisma.user.count.mockResolvedValueOnce(totalUsers).mockResolvedValueOnce(activeUsers);
    mockPrisma.course.count.mockResolvedValueOnce(totalCourses).mockResolvedValueOnce(publishedCourses);
  ...
  describe('getTenants', () => {
    ...
    it('should filter tenants by plan', async () => {
      ...
      await service.getTenants(1, 20, undefined, _client.SubscriptionPlan.ENTERPRISE);
      expect(mockPrisma.tenant.findMany).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({
          plan: _client.SubscriptionPlan.ENTERPRISE
        })
      }));
    });
  ...
  describe('getTenantDetails', () => {
    ...
    it('should throw NotFoundException when tenant does not exist', async () => {
      ...
      await expect(service.getTenantDetails('nonexistent')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('updateTenantLegacy', () => {
    it('should update tenant fields', async () => {
      ...
      const result = await service.updateTenantLegacy('t-1', {
        name: 'New Name',
        plan: _client.SubscriptionPlan.PROFESSIONAL
      }, 'admin-1');
    ...
    it('should throw NotFoundException when tenant does not exist', async () => {
      mockPrisma.tenant.findUnique.mockResolvedValueOnce(null);
      await expect(service.updateTenantLegacy('nonexistent', {
        name: 'X'
      }, 'admin-1')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('deleteTenant', () => {
    ...
    it('should throw NotFoundException when tenant does not exist', async () => {
      mockPrisma.tenant.findUnique.mockResolvedValueOnce(null);
      await expect(service.deleteTenant('nonexistent', 'admin-1')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('getUsers', () => {
    ...
    it('should filter users by role', async () => {
      ...
      await service.getUsers(1, 20, undefined, _client.UserRole.TEACHER);
  ...
  describe('updateUserLegacy', () => {
    it('should update user role and active status', async () => {
      ...
      const result = await service.updateUserLegacy('u-1', {
        role: _client.UserRole.TEACHER,
        isActive: false
      }, 'admin-1');
    ...
    it('should throw NotFoundException when user does not exist', async () => {
      ...
      await expect(service.updateUserLegacy('nonexistent', {
        isActive: false
      }, 'admin-1')).rejects.toThrow(_common.NotFoundException);
    });
  });
  ...
  describe('impersonate', () => {
    ...
    it('should throw NotFoundException when target user does not exist', async () => {
      mockPrisma.user.findUnique.mockResolvedValueOnce(null);
      await expect(service.impersonate('nonexistent')).rejects.toThrow(_common.NotFoundException);
    });
  });
