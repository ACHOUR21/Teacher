...
describe('GdprService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_gdpr.GdprService, {
        provide: _prisma.PrismaService,
        useValue: mockPrisma
      }]
    }).compile();
    service = module.get(_gdpr.GdprService);
    jest.clearAllMocks();
  });
  ...
  describe('exportUserData', () => {
    it('should return a structured export without sensitive fields', async () => {
      ...
      expect(result.data.mfaSecret).toBeUndefined();
      expect(result.data.mfaBackupCodes).toBeUndefined();
      // Safe fields should be present
      expect(result.data.email).toBe('alice@example.com');
    });
    it('should throw NotFoundException when user does not exist', async () => {
      mockPrisma.user.findUnique.mockResolvedValueOnce(null);
      await expect(service.exportUserData('nonexistent-id')).rejects.toThrow(_common.NotFoundException);
    });
  });
  // ─── upsertConsents ───────────────────────────────────────────────────────
  describe('upsertConsents', () => {
    it('should create new consent records', async () => {
      ...
      await service.upsertConsents('user-1', 'tenant-1', [{
        type: _client.ConsentType.MARKETING,
        granted: true
      }], {
        ipAddress: '127.0.0.1',
      ...
      expect(mockPrisma.userConsent.upsert).toHaveBeenCalledWith(expect.objectContaining({
        where: {
          userId_type: {
            userId: 'user-1',
            type: _client.ConsentType.MARKETING
    ...
    it('should update existing consent to revoked', async () => {
      mockPrisma.userConsent.upsert.mockResolvedValue({});
      await service.upsertConsents('user-1', 'tenant-1', [{
        type: _client.ConsentType.ANALYTICS,
        granted: false,
        version: '2.0'
      }], {});
      expect(mockPrisma.userConsent.upsert).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('cancelDeletion', () => {
    it('should cancel a pending deletion request and re-enable the user', async () => {
      ...
      expect(mockPrisma.dataDeletionRequest.update).toHaveBeenCalledWith(expect.objectContaining({
        data: {
          status: 'CANCELLED'
        }
      }));
    ...
    it('should throw BadRequestException when no pending request exists', async () => {
      mockPrisma.dataDeletionRequest.findUnique.mockResolvedValueOnce(null);
      await expect(service.cancelDeletion('user-1')).rejects.toThrow(_common.BadRequestException);
    });
