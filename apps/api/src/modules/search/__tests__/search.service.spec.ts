...
describe('SearchService', () => {
  ...
  describe('indexCourse', () => {
    it('should index a course document', async () => {
      ...
      expect(mockEs.index).toHaveBeenCalledWith(
        expect.objectContaining({ index: 'courses', id: 'c-1' }),
      );
    });
    ...
    it('should silently handle indexing errors', async () => {
      ...
      await expect(
        service.indexCourse({ id: 'c-1', title: 'Test', tags: [], teacherName: 'T', tenantId: 't-1' }),
  ...
  describe('searchCourses', () => {
    it('should return search hits with pagination info', async () => {
      mockEs.search.mockResolvedValueOnce({
        hits: {
          hits: [
            { _id: 'c-1', _score: 1.5, _source: { title: 'Algebra I', tenantId: 'tenant-1' }, highlight: { title: ['<em>Algebra</em>'] } },
          ],
          total: { value: 1 },
        },
      });
    ...
    it('should apply category filter', async () => {
      mockEs.search.mockResolvedValueOnce({ hits: { hits: [], total: { value: 0 } } });

      await service.searchCourses('math', 'tenant-1', { category: 'Mathematics' });
          ...
          query: expect.objectContaining({
            bool: expect.objectContaining({
              must: expect.arrayContaining([
                expect.objectContaining({ term: { category: 'Mathematics' } }),
              ]),
            }),
          }),
        }),
  ...
  describe('deleteDocument', () => {
    it('should delete a document by index and id', async () => {
      ...
      expect(mockEs.delete).toHaveBeenCalledWith({ index: 'courses', id: 'c-1' });
    });
  ...
  describe('createIndices', () => {
    it('should create index if it does not exist', async () => {
      ...
      expect(mockEs.indices.create).toHaveBeenCalledWith(
        expect.objectContaining({ index: 'courses' }),
      );
    });
