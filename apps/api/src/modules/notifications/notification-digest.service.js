...
var _email = require("./email/email.service");
...
var NotificationDigestService_1;
var _a;
let NotificationDigestService = exports.NotificationDigestService = NotificationDigestService_1 = class NotificationDigestService {
  logger = new _common.Logger(NotificationDigestService_1.name);
  ...
  async sendWeeklyDigests() {
    ...
    for (const user of users) {
      try {
        const prefs = user.profile?.preferences?.['notificationPreferences'];
  ...
  async buildDigestForUser(userId) {
  ...
  async sendDigestEmail(digest) {
    ...
    const courseRows = digest.coursesProgressed.length ? digest.coursesProgressed.map(c => `
          <tr>
          ...
          </tr>`).join('') : '<tr><td colspan="2" style="color:#9ca3af;font-size:14px;padding:8px 0">No course activity this week.</td></tr>';
    const achievementRows = digest.achievementsUnlocked.length ? digest.achievementsUnlocked.map(a => `<li style="margin:4px 0;color:#374151;font-size:14px">
            <span style="margin-right:6px">🏆</span>${a.title}</li>`).join('') : '<li style="color:#9ca3af;font-size:14px">No new achievements this week.</li>';
    const upcomingRows = digest.upcomingLessons.length ? digest.upcomingLessons.map(s => {
    ...
    }).join('') : '<li style="color:#9ca3af;font-size:14px">No upcoming sessions.</li>';
...
__decorate([(0, _schedule.Cron)('0 8 * * 1'), __metadata("design:type", Function), __metadata("design:paramtypes", []), __metadata("design:returntype", typeof (_a = typeof Promise !== "undefined" && Promise) === "function" ? _a : Object)], NotificationDigestService.prototype, "sendWeeklyDigests", null);
exports.NotificationDigestService = NotificationDigestService = NotificationDigestService_1 = __decorate([(0, _common.Injectable)(), __param(0, (0, _common.Inject)(_prisma.PrismaService)), __param(1, (0, _common.Inject)(_email.EmailService)), __metadata("design:paramtypes", [Object, Object])], NotificationDigestService);