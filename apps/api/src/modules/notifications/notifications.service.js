...
let NotificationsService = exports.NotificationsService = NotificationsService_1 = class NotificationsService {
  logger = new _common.Logger(NotificationsService_1.name);
  transporter;
  ...
  async sendPush(fcmToken, title, body, data) {
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.error('FCM push failed', await response.text());
      }
    } catch (err) {
      this.logger.error('FCM push error', err);
    }
  }
  async createInApp(userId, type, title, body, data) {
  ...
  async notifyUser(userId, title, body, data) {
    ...
    const notification = await this.createInApp(userId, _client.NotificationType.IN_APP, title, body, data);
    ...
    for (const device of user.devices) {
      if (device.fcmToken) {
        await this.sendPush(device.fcmToken, title, body, data);
      }
    }
  }
  async registerFcmToken(userId, fcmToken, platform = 'WEB') {
  ...
  async sendWelcomeEmail(to, firstName, tenantName) {
  ...
  async sendCourseEnrollmentEmail(to, firstName, courseTitle, courseId) {
  ...
  async sendGradePublishedEmail(to, firstName, assignmentTitle, grade, maxGrade, feedback) {
  ...
  async sendAssignmentDueReminderEmail(to, firstName, assignmentTitle, courseTitle, dueDate) {
  ...
  async sendCertificateAwardedEmail(to, firstName, courseTitle, verifyCode) {
  ...
  async sendAssignmentSubmittedEmail(to, instructorName, studentName, assignmentTitle, courseTitle) {
    const html = emailTemplate({
  ...
  async sendCourseCompletionEmail(to, firstName, courseTitle, courseId) {
    const html = emailTemplate({
  ...
  async sendAdminAlertEmail(to, adminName, alertTitle, alertBody) {
    const html = emailTemplate({
      title: alertTitle,
...
exports.NotificationsService = NotificationsService = NotificationsService_1 = __decorate([(0, _common.Injectable)(), __param(0, (0, _common.Inject)(_prisma.PrismaService)), __param(1, (0, _common.Optional)()), __metadata("design:paramtypes", [Object, Object])], NotificationsService);
// ── Shared email layout ──────────────────────────────────────────────────────
