...
describe('RedisService', () => {
  ...
  describe('getObject', () => {
    it('should deserialize a stored JSON object', async () => {
      redisClient.get.mockResolvedValueOnce(JSON.stringify({ name: 'Alice', age: 30 }));

      const result = await service.getObject<{ name: string; age: number }>('user:1');
  ...
  describe('setObject', () => {
    it('should serialize and store an object', async () => {
      ...
      await service.setObject('user:1', { name: 'Alice' }, 60);
