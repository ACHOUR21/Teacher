...
describe('UniversityErpService', () => {
  let service: UniversityErpService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        UniversityErpService,
        { provide: PrismaService, useValue: mockPrisma },
      ],
    }).compile();

    service = module.get<UniversityErpService>(UniversityErpService);
    jest.clearAllMocks();
  });

  describe('createUniversity', () => {
    it('should create a university for a tenant', async () => {
      const uni = { id: 'uni-1', tenantId: 'tenant-1', name: 'State University' };
      mockPrisma.university.create.mockResolvedValueOnce(uni);

      const result = await service.createUniversity('tenant-1', { name: 'State University' });
  ...
  describe('getUniversities', () => {
    it('should return active universities with faculty and program counts', async () => {
      const unis = [
        { id: 'uni-1', name: 'State Uni', isActive: true, _count: { faculties: 8, programs: 25 } },
      ];
  ...
  describe('createFaculty', () => {
    it('should create a faculty within a university', async () => {
      const faculty = { id: 'fac-1', universityId: 'uni-1', name: 'Engineering' };
      mockPrisma.faculty.create.mockResolvedValueOnce(faculty);

      const result = await service.createFaculty('uni-1', { name: 'Engineering', code: 'ENG' });
  ...
  describe('createProgram', () => {
    it('should create an academic program', async () => {
      const program = { id: 'prog-1', universityId: 'uni-1', name: 'Computer Science', degree: 'BSc', durationYears: 4 };
  ...
  describe('getPrograms', () => {
    it('should return programs with department and enrollment count', async () => {
      const programs = [
        { id: 'prog-1', name: 'CS', degree: 'BSc', department: { name: 'CS Dept' }, _count: { enrollments: 120 } },
      ];
  ...
  describe('enrollStudent', () => {
    it('should enroll a student in a program', async () => {
      const enrollment = { id: 'enr-1', studentId: 's-1', programId: 'prog-1', status: EnrollmentStatus.ACTIVE };
  ...
  describe('updateEnrollmentStatus', () => {
    it('should update enrollment status to COMPLETED with completedAt', async () => {
      const updated = { id: 'enr-1', status: EnrollmentStatus.COMPLETED, completedAt: new Date() };
      mockPrisma.enrollment.update.mockResolvedValueOnce(updated);

      const result = await service.updateEnrollmentStatus('enr-1', EnrollmentStatus.COMPLETED);
    ...
    it('should update enrollment status to WITHDRAWN without completedAt', async () => {
      const updated = { id: 'enr-1', status: EnrollmentStatus.WITHDRAWN };
      mockPrisma.enrollment.update.mockResolvedValueOnce(updated);

      const result = await service.updateEnrollmentStatus('enr-1', EnrollmentStatus.WITHDRAWN);
