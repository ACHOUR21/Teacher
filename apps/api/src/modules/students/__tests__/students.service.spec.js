...
describe('StudentsService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_students.StudentsService, {
    ...
    service = module.get(_students.StudentsService);
    jest.clearAllMocks();
  });
  describe('findAll', () => {
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException for unknown student', async () => {
      ...
      await expect(service.findOne('unknown')).rejects.toThrow(_common.NotFoundException);
    });
  });
  describe('getPerformanceSummary', () => {
    it('should compute course progress and submission stats', async () => {
      ...
      mockPrisma.userPoints.findUnique.mockResolvedValueOnce({
        total: 450,
        level: 3
      });
      mockPrisma.submission.aggregate.mockResolvedValueOnce({
        _avg: {
          score: 85
        }
      });
