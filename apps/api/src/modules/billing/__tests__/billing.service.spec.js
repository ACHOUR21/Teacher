...
describe('BillingService', () => {
  ...
  beforeEach(async () => {
    const module = await _testing.Test.createTestingModule({
      providers: [_billing.BillingService, {
        ...
        useValue: mockApiEcosystem
      }]
    }).compile();
    service = module.get(_billing.BillingService);
    jest.clearAllMocks();
  });
  ...
  describe('subscribe', () => {
    ...
    it('should create a new Stripe customer when none exists and return checkoutUrl', async () => {
      ...
      const result = await service.subscribe('tenant-1', 'PROFESSIONAL', 'user-1', SUCCESS_URL, CANCEL_URL);
    ...
    it('should reuse existing Stripe customer and not call customers.create', async () => {
      ...
      mockPrisma.tenant.findUnique.mockResolvedValueOnce(mockTenant);
      mockPrisma.user.findUnique.mockResolvedValueOnce(mockUser);
      await service.subscribe('tenant-1', 'PROFESSIONAL', 'user-1', SUCCESS_URL, CANCEL_URL);
    ...
    it('should throw NotFoundException when tenant does not exist', async () => {
      ...
      await expect(service.subscribe('bad-tenant', 'PROFESSIONAL', 'user-1', SUCCESS_URL, CANCEL_URL)).rejects.toThrow(_common.NotFoundException);
    });
    it('should throw BadRequestException for FREE_TRIAL plan (no price ID)', async () => {
      ...
      mockPrisma.tenant.findUnique.mockResolvedValueOnce(mockTenant);
      mockPrisma.user.findUnique.mockResolvedValueOnce(mockUser);
      await expect(service.subscribe('tenant-1', 'FREE_TRIAL', 'user-1', SUCCESS_URL, CANCEL_URL)).rejects.toThrow(_common.BadRequestException);
    });
  });
  describe('cancelSubscription', () => {
    ...
    it('should throw NotFoundException when no active subscription exists', async () => {
      ...
      await expect(service.cancelSubscription('tenant-1')).rejects.toThrow(new _common.NotFoundException('No active subscription found'));
    });
  });
  ...
  describe('applyCoupon', () => {
    ...
    it('should throw NotFoundException when coupon does not exist', async () => {
      ...
      await expect(service.applyCoupon('tenant-1', 'INVALID')).rejects.toThrow(new _common.NotFoundException('Coupon not found'));
    });
    it('should throw BadRequestException when coupon is inactive', async () => {
      ...
      await expect(service.applyCoupon('tenant-1', 'SAVE10')).rejects.toThrow(new _common.BadRequestException('Coupon is no longer active'));
    });
    it('should throw BadRequestException when coupon has expired', async () => {
      ...
      await expect(service.applyCoupon('tenant-1', 'SAVE10')).rejects.toThrow(new _common.BadRequestException('Coupon has expired'));
    });
    it('should throw BadRequestException when coupon usage limit is reached', async () => {
      ...
      await expect(service.applyCoupon('tenant-1', 'SAVE10')).rejects.toThrow(new _common.BadRequestException('Coupon usage limit reached'));
    });
    it('should successfully apply coupon and return discount info', async () => {
      ...
      mockPrisma.subscription.findUnique.mockResolvedValueOnce(null);
      mockPrisma.coupon.update.mockResolvedValueOnce({
        ...validCoupon,
        usedCount: 1
      });
    ...
    it('should apply coupon to Stripe subscription when both stripeSubscriptionId and stripeCouponId exist', async () => {
      const couponWithStripe = {
        ...validCoupon,
        stripeCouponId: 'stripe_coupon_abc'
      };
  ...
  describe('getInvoices', () => {
    it('should throw NotFoundException when no subscription exists', async () => {
      mockPrisma.subscription.findUnique.mockResolvedValueOnce(null);
      await expect(service.getInvoices('tenant-1')).rejects.toThrow(new _common.NotFoundException('No subscription found'));
    });
  ...
  describe('createCourseCheckoutSession', () => {
    it('should throw NotFoundException when course does not exist', async () => {
      ...
      await expect(service.createCourseCheckoutSession('user-1', 'course-404', 'https://success.url', 'https://cancel.url')).rejects.toThrow(new _common.NotFoundException('Course not found'));
    });
    it('should throw ConflictException when student is already enrolled in the course', async () => {
      ...
      await expect(service.createCourseCheckoutSession('user-1', 'course-1', 'https://success.url', 'https://cancel.url')).rejects.toThrow(new _common.ConflictException('Already enrolled in this course'));
    });
    it('should return checkoutUrl and sessionId on success', async () => {
      ...
      mockPrisma.course.findUnique.mockResolvedValueOnce(mockCourse);
      mockPrisma.student.findFirst.mockResolvedValueOnce(null);
      const result = await service.createCourseCheckoutSession('user-1', 'course-1', 'https://success.url', 'https://cancel.url');
      expect(mockStripe.checkout.sessions.create).toHaveBeenCalledWith(expect.objectContaining({
        mode: 'payment',
        metadata: {
          userId: 'user-1',
  ...
  describe('createPortalSession — error path', () => {
    it('should throw NotFoundException when no billing account exists', async () => {
      mockPrisma.subscription.findUnique.mockResolvedValueOnce(null);
      await expect(service.createPortalSession('tenant-1', 'https://app.example.com/billing')).rejects.toThrow(new _common.NotFoundException('No billing account found'));
    });
  });
});